...
| 接口 | 接口类型 | 请求参数 | 返回值 | 说明 |
|------|---------|---------|--------|------|
| 发送消息 | 同步请求 | 用户消息文本 + 传输方式选择(流式/整包) | AI回复内容 | 用户发消息后在线等结果。流式时通过SSE通道逐块接收（事件类型：text/tool/final），整包时通过HTTP响应一次性接收 |
...
> 说明："接收流式回复"和"接收整包回复"是"发送消息"的传输方式选项，不是独立接口。流式时通过SSE通道逐块接收（事件类型：text/tool/final），请求参数为会话ID/请求ID；整包时通过HTTP响应一次性接收。

**对下的接口（第1层调用第2层）**：

| 接口 | 调用方向 | 请求参数 | 返回值 | 说明 |
|------|---------|---------|--------|------|
| 处理同步请求 | 第1层→第2层 | 用户消息 + 传输方式 | 完整AI回复 | 转发同步请求给编排层，传递用户选择的传输方式 |
...
| 前端功能 | 用户操作 | 所属层次 | 说明 |
|---------|---------|---------|------|
| 发送消息 | 用户输入文字后指定传输方式（流式/整包）| 第1层：用户入口层 | 流式时通过SSE通道逐块接收，整包时一次性接收 |
