...
| **逐段输出过程中可插入工具调用** | AI在逐段输出中可随时调用工具，工具执行后继续输出 |
...
注：整包路径的决策会影响0.2"严格调用"原则的适用范围——若选B，则需在该原则中增加"整包快捷路径例外"条款。
...
| 前端功能 | 用户操作 | 所属层次 | 说明 |
|---------|---------|---------|------|
| 发送消息 | 用户输入文字后发送 | 第1层：用户入口层 | 同步请求入口接收 |
| 接收回复 | 实时看到逐段输出 | 第1层：用户入口层 | 同步请求入口返回流式结果 |
| 接收完整回复 | 等待后一次性看到结果 | 第1层：用户入口层 | 同步请求入口返回整包结果 |
...
| 继续对话 | 任务完成后追问 | 第1层：用户入口层 | 新的同步请求，携带上下文 |
...
| 功能 | 说明 |
|------|------|
| 构建流式请求 | 构建符合OpenAI格式的流式请求体（stream=true）→ 发送到LLM API → 逐块接收 → 解析文本块和工具调用块 → 返回结构化结果 |
| 构建整包请求 | 构建符合OpenAI格式的整包请求体 → 发送到LLM API → 等待完整响应 → 返回结构化结果 |
| 模型能力适配 | 不同模型的工具调用能力不同 → 统一接口屏蔽差异 → 第2层无需感知具体模型差异 |
| 网络级重试 | 检测网络异常（超时、限流429、连接断开） → 自动重试 → 重试失败则向第2层报告 |
...
| 不解析工具调用语义 | 只负责从LLM响应中提取工具调用参数，不判断工具是否应该执行 |
...
１　基于第２章的概要设计，　详细设计每一层的函数功能　及函数功能的对应代码文件
２　这些都是函数模块　　不使用对象设计模型？因为每一层自己就是调用关系　没有办法是基类继承的方式实现？
３
４
...
##　三 新正确架构的详细设计［第一层］－－代码级＋函数［就是　哪些代码各个代码都什么功能］
　

### ３.1 新架构代码结构－－［需要哪些代码文件］


### ３.２ 新架构代码结构－－［需要哪些代码文件＝函数］
