...
>
> **v1.7更新说明 (2026-02-17 23:00:00)**：
> - 修正API实现状态（validate、switch已实现）
> - 添加缺失的第8章（数据库设计）
> - 修正AC数量（10个，不是7个）
>
> **v1.6更新说明 (2026-02-17 22:50:00)**：
> - 修复重复章节编号（两个## 11 → ## 11和## 12）
>
> **v1.5更新说明 (2026-02-17 21:00:00)**：
> - 修复章节编号系统性错误（5.x → 7.x, 8.x → 9.x, 9.x → 10.x）
> - 更新API接口设计，标注实际实现状态
> - 更新安全机制实现状态（已实现vs待实现）
> - 更新前端组件实现状态对照表
> - 修复验收标准数量不一致（AC007→AC010）  
...
│   ├── hooks/                    ✅ 已实现（功能集成在组件中）
│   │   ├── useChat.ts            # 聊天逻辑集成在Chat/index.tsx
│   │   ├── useExecution.ts       # 执行逻辑集成在ExecutionPanel.tsx
│   │   └── useWebSocket.ts       # SSE逻辑在utils/sse.ts
│   ├── services/
│   │   ├── api.ts               ✅ 已实现
│   │   └── chatService.ts       ✅ 已实现（集成在api.ts）
│   ├── types/
│   │   └── chat.ts              ✅ 已实现
│   ├── utils/
│   │   └── format.ts            ⚠️预留
...
| 组件/模块 | 状态 | 说明 |
|----------|------|------|
| `Chat/index.tsx` | ✅ 已实现 | 聊天主组件（合并设计中的多个组件） |
| `MessageList` | ✅ 已实现 | 消息列表（集成在Chat组件中） |
| `MessageItem` | ✅ 已实现 | 单条消息展示 |
| `MessageInput` | ✅ 已实现 | 消息输入框 |
...
POST /api/v1/chat
Request: {
  messages: ChatMessage[];     // 消息列表 [{role: "user"|"assistant", content: string}]
  stream?: boolean;           // 是否流式返回（当前不支持）
...
Response: {
  ...
  provider: string;          // 当前提供商 (zhipuai/opencode)
...
| 接口 | 状态 | 说明 |
|------|------|------|
| `POST /api/v1/chat` | ✅ 已实现 | 支持文件操作意图检测 |
| `GET /api/v1/chat/validate` | ✅ 已实现 | 验证API Key有效性 |
| `POST /api/v1/chat/switch/{provider}` | ✅ 已实现 | 动态切换AI提供商 |
| `GET /api/v1/chat/execution/:session_id` | ⚠️ 部分实现 | Agent返回结果，但无独立查询接口 |

### 4.2 会话管理

> ✅ 会话管理功能已实现，createSession已对接真实API（非Mock）

```typescript
// 创建会话 ✅已实现
POST /api/v1/sessions
// Response: { session_id: string, title: string, created_at: string, updated_at: string, message_count: number }
...
| 接口 | 状态 | 说明 |
|------|------|------|
| `POST /api/v1/sessions` | ✅ 已实现 | 已对接真实API，返回完整会话信息 |
| `GET /api/v1/sessions` | ✅ 已实现 | 支持分页和关键字搜索 |
| `DELETE /api/v1/sessions/:session_id` | ✅ 已实现 | 支持单条删除，清空会话通过循环调用实现 |
...
// 获取配置 ✅已实现
GET /api/v1/config
// Response: { ai_provider, ai_model, api_key_configured, theme, language, security }

// 更新配置 ✅已实现
PUT /api/v1/config
// Request: { ai_provider?, zhipu_api_key?, opencode_api_key?, theme?, security? }
// Response: { success: boolean, message: string }

// 验证配置 ✅已实现
POST /api/v1/config/validate
// Request: { provider, api_key }
// Response: { valid: boolean, message: string, model?: string }
```
...
| `PUT /api/v1/config` | ✅ 已实现 | 支持模型配置和安全配置更新 |
| `POST /api/v1/config/validate` | ✅ 已实现 | 验证API Key有效性 |

### 4.4 安全接口

> ⚠️ 安全模块已实现操作历史、回收站、回滚功能，但Shell黑名单机制待开发。

```typescript
// 获取安全策略 (部分实现)
GET /api/v1/security/policy
// Response: {
//   shell_blacklist: string[];  // Shell黑名单命令 - 待实现
//   dangerous_operations: string[];  // 危险操作列表 - 待实现
// }

// 危险操作确认 (未实现)
POST /api/v1/security/confirm
// Request: { operation_id: string, confirmed: boolean }
```

**实现状态**：
| 接口 | 状态 | 说明 |
|------|------|------|
| 安全服务-操作记录 | ✅ 已实现 | SQLite记录所有操作 |
| 安全服务-回收站 | ✅ 已实现 | 删除文件自动备份 |
| 安全服务-回滚 | ✅ 已实现 | 支持单操作/会话回滚 |
| Shell黑名单 | ❌ 未实现 | 需开发 |
| 危险操作确认 | ❌ 未实现 | 需开发 |
...
**已实现**：
- [x] 全局错误捕获 (FastAPI异常处理器)
- [x] 友好的错误提示 (chat.py错误处理)
- [x] 日志记录机制 (app/utils/logger.py)
- [x] 操作历史记录 (SQLite数据库)
- [x] 回收站机制 (自动备份删除文件)
- [x] 回滚功能 (单操作/会话回滚)
...
| Day 5 | 验收测试、文档完善 | 阶段文档完成 | AC001-AC010全部通过 |
...
CREATE INDEX idx_sessions_deleted ON chat_sessions(is_deleted);
```

### 8.2 消息表 (chat_messages)
...
CREATE TABLE chat_messages (
    ...
    execution_steps JSON,                  -- 执行过程JSON（可选）
...
| **验收测试** | 10个AC (AC001-AC010) | 全部自动化 | Playwright |
| **总计** | 70个左右 | 整体>80% | - |
