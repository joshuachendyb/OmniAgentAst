...
## 第十章 配置化方案设计（内置+配置文件模式）

**更新时间**: 2026-02-20 13:51:43  
**更新人**: 小沈

### 10.1 设计背景

为了提高 CRSS（命令风险评分系统）的灵活性和可维护性，我们将原有的硬编码配置改为**内置默认+配置文件覆盖**的混合模式，使得：
- 系统开箱即用（内置默认配置）
- 用户可自定义配置（配置文件覆盖）
- 保持向后兼容
...
| 配置项 | 用途 | 加载优先级 |
|--------|------|-----------|
| **DANGEROUS_COMMANDS** | 危险命令黑名单 | 配置文件 > 内置默认 |
| **SYSTEM_CRITICAL_DIRS** | 系统关键目录 | 配置文件 > 内置默认 |
| **PROJECT_PROTECTED_DIRS** | 项目保护目录 | 配置文件 > 内置默认 |

### 10.3 配置文件结构

配置文件位置：`config/security_config.yaml`
...
**加载逻辑**：
1. 首先尝试加载 `config/security_config.yaml`
2. 如果文件不存在或加载失败，使用内置默认配置
3. 配置文件中的值会覆盖内置默认值
4. 配置数据会缓存在内存中，避免重复读取
...
from app.services.shell_security import (
    get_security_config,
    get_system_critical_dirs,
    get_project_protected_dirs
)
...
# 获取系统关键目录（用于检测系统目录操作）
system_dirs = get_system_critical_dirs()

# 获取项目保护目录（用于检测项目目录操作）
project_dirs = get_project_protected_dirs()
```
...
**SYSTEM_CRITICAL_DIRS 默认值**：
- Windows: `C:\Windows`, `C:\Windows\System32`, `C:\Program Files` 等
- Linux: `/bin`, `/sbin`, `/usr/bin`, `/etc`, `/proc`, `/dev` 等

**PROJECT_PROTECTED_DIRS 默认值**：
- 代码目录: `backend/`, `frontend/`, `src/`, `tests/`
- 文档目录: `doc/`, `doc-阶段2.1/`, `笔记目录/`
- 配置目录: `.git/`, `package.json`, `.env`
