...
test.describe('Settings Page - Model Configuration', () => {
  ...
  test('should have provider dropdown with options', async ({ page }) => {
    ...
    try {
      ...
      const hasOptions = await Promise.any([
        page.getByText(/智谱AI/).isVisible().catch(() => false),
        page.getByText(/OpenCode/).isVisible().catch(() => false),
        page.locator('.ant-select-item-option').first().isVisible().catch(() => false),
      ]).catch(() => false);
  ...
  test('should have temperature slider options', async ({ page }) => {
    ...
    const hasTemperature = await Promise.any([
      page.getByText(/温度/).isVisible().catch(() => false),
      page.locator('.ant-slider').isVisible().catch(() => false),
    ]).catch(() => false);
...
test.describe('Settings Page - Security Configuration', () => {
  ...
  test('should display security switches', async ({ page }) => {
    ...
    const hasSecurity = await Promise.any([
      page.getByText(/安全/).isVisible().catch(() => false),
      page.locator('.ant-switch').isVisible().catch(() => false),
    ]).catch(() => false);
    
    // At least one security element should exist
    expect(hasSecurity || await page.locator('.ant-switch').count() > 0).toBe(true);
  });
...
test.describe('Settings Page - Session History', () => {
  ...
  test('should display session list controls', async ({ page }) => {
    ...
    const hasControls = await Promise.any([
      page.getByText(/会话/).isVisible().catch(() => false),
      page.getByRole('button').isVisible().catch(() => false),
    ]).catch(() => false);
    
    expect(hasControls || await page.getByRole('button').count() > 0).toBe(true);
  });
  ...
  test('should show empty state when no sessions', async ({ page }) => {
    ...
    const hasContent = await Promise.any([
      page.getByText(/暂无/).isVisible().catch(() => false),
      page.locator('.ant-list').isVisible().catch(() => false),
    ]).catch(() => false);
  ...
  test('should show confirmation before clearing all sessions', async ({ page }) => {
    ...
    try {
      ...
      const hasDialog = await Promise.any([
        page.getByText(/确定/).isVisible().catch(() => false),
        page.locator('.ant-modal').isVisible().catch(() => false),
      ]).catch(() => false);
