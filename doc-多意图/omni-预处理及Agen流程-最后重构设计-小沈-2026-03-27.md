...
| v2.84 | 2026-03-27 10:15:00 | 修正架构描述：步骤6改为"调用react_sse_wrapper"，分发逻辑在react_sse_wrapper内部，新增附录2.3.6分发逻辑说明 |
| v2.85 | 2026-03-27 10:25:00 | 修正外部调用架构图：分发逻辑在第二层内部，修正第一层只调用react_sse_wrapper |
| v2.86 | 2026-03-27 13:55:00 | 修正附录6-3.4节错误引用：阶段3改为阶段6，ver1_run_stream改为run_stream |
| v2.87 | 2026-03-27 14:07:14 | 全文一致性修正：修正阶段1描述(ver1_run_stream→run_stream)、附录6-3.1当前调用链、版本历史等多处与附录6-3.2实施步骤不一致的地方 |
| v2.88 | 2026-03-27 14:30:00 | 阶段6/7完整化：补充阶段7实现代码示例，删除7.4废弃agent.py章节，修正当前待办引用 |
...
└─────────────────────────────────────────────────────────────────┘
               ...
               ▼
┌─────────────────────────────────────────────────────────────────┐
│  react_sse_wrapper.py (第二层)                                   │
│                                                                 │
│  **根据 intent_type 分发到不同Agent：**                          │
│    ├─ intent=chat → chat_stream_query                           │
│    ├─ intent=file → FileReactAgent                              │
│    ├─ intent=network → NetworkReactAgent                        │
│    └─ intent=desktop → DesktopReactAgent                        │
│                                                                 │
│  - SSE格式化、任务管理、数据库保存、中断检查                      │
└─────────────────────────────────────────────────────────────────┘
               │
     ┌────────┼────────┐
     │        │        │
...
| 6 | 调用react_sse_wrapper | react_sse_wrapper内部根据intent_type分发到不同Agent |
...
| 文件 | 职责 | 不做 |
|------|------|------|
| **chat_router.py** | API端点 + 6步流程：预处理→意图检测→初始化→安全检测→start→**调用react_sse_wrapper** | 具体执行 |
...
| 层次 | 文件 | 职责 | 特点 |
|------|------|------|------|
| **路由层** | chat_router.py | 6步完整流程：预处理→意图检测→初始化→安全检测→start→**调用react_sse_wrapper** | 路由入口 |
...
chat_router (第一层)
     ↓ 调用
react_sse_wrapper (第二层) - SSE包装+分发
     ↓ 分发到第三层
┌────────────┬────────────┬────────────┬────────────┐
↓            ↓            ↓            ↓            ↓
FileReact  Network   chat_stream_query  Desktop      (第三层Agent)
  ↓          ↓          ↓          ↓
base_react base_react (直接流式) base_react     (第四层: base_react)
```
...
| 层 | 文件 | 职责 | 特点 |
|---|------|------|------|
| **路由层** | chat_router.py | 6步完整流程：预处理→意图检测→初始化→安全检测→start→**调用react_sse_wrapper** | 新增 |
...
4. **不再需要现有的 agent.py 作为独立文件**
   ...
   - chat_router 负责调用 react_sse_wrapper，分发逻辑在 react_sse_wrapper 内部

### 附录2.3.6 react_sse_wrapper 分发逻辑说明

> **整理时间**: 2026-03-27
> **整理人**: 小沈
>
> **说明**：react_sse_wrapper 是第二层，负责根据 intent_type 分发到不同的处理函数

**react_sse_wrapper 的分发逻辑**：

| intent_type | 条件 | 调用函数 | 说明 |
|-------------|------|----------|------|
| chat | intent_type=="chat" 或 confidence < 0.3 | chat_stream_query() | 简单对话流（已返回SSE） |
| file | intent_type=="file" 且 confidence >= 0.3 | FileReactAgent.run_stream() | 文件操作ReAct（返回event dict） |
| network | intent_type=="network" 且 confidence >= 0.3 | NetworkReactAgent.run_stream() | 网络操作ReAct（返回event dict） |
| desktop | intent_type=="desktop" 且 confidence >= 0.3 | DesktopReactAgent.run_stream() | 桌面操作ReAct（返回event dict） |
| 默认 | 其他情况 | chat_stream_query() | 回退到简单对话 |

**分发代码示例**：
```python
# react_sse_wrapper 内部的分发逻辑
if intent_type == "chat" or confidence < 0.3:
    # 调用 chat_stream_query（已返回SSE）
    async for event in chat_stream_query(...):
        yield event
elif intent_type == "file" and confidence >= 0.3:
    # 调用 FileReactAgent.run_stream()（返回event dict）
    async for event in agent.run_stream(...):
        yield _format_sse(event, step)  # SSE格式化
elif intent_type == "network" and confidence >= 0.3:
    # 调用 NetworkReactAgent.run_stream()（返回event dict）
    async for event in agent.run_stream(...):
        yield _format_sse(event, step)
elif intent_type == "desktop" and confidence >= 0.3:
    # 调用 DesktopReactAgent.run_stream()（返回event dict）
    async for event in agent.run_stream(...):
        yield _format_sse(event, step)
else:
    # 默认回退到 chat_stream_query
    async for event in chat_stream_query(...):
        yield event
```

**关键理解**：
- 第一层（chat_router）只负责调用 react_sse_wrapper
- 分发逻辑在第二层（react_sse_wrapper）内部
- react_sse_wrapper 负责：
  1. SSE 格式化（dict → SSE 字符串）
  2. 任务管理（running_tasks）
  3. 数据库保存（save_execution_steps_to_db）
  4. 中断/暂停检查
  5. **根据 intent_type 分发到不同的 Agent**
...
| **调用react_sse_wrapper** | 步骤6 | ✅ |
| 返回统一的 SSE 流 | yield sse_data | ✅ |

**react_sse_wrapper 内部根据 intent_type 分发到不同Agent**：
| intent_type | 调用函数 | 状态 |
|-------------|----------|------|
| chat / confidence < 0.3 | chat_stream_query() | ✅ 已实现 |
| file / confidence >= 0.3 | FileReactAgent | ✅ 已实现 |
| network / confidence >= 0.3 | NetworkReactAgent | ❌ 待实现 |
| desktop / confidence >= 0.3 | DesktopReactAgent | ❌ 待实现 |
...
- ✅ 实现意图分发：调用 react_sse_wrapper.generate_sse_stream()，由 react_sse_wrapper 内部根据 intent_type 分发到不同Agent
- ✅ 语法检查通过
- ✅ 32个测试全部通过
...
| ⚠️ start步骤由第一层发送，本层不重复发送 | react_sse_wrapper不含start逻辑 |
...
```
阶段1：chat_router → react_sse_wrapper（第二层调用）【参考附录2.5章节操作说明】
       ├── 实现 chat_router.py（第一层）
       └── 调用 react_sse_wrapper.generate_sse_stream()（由第二层内部根据intent_type分发）
       验证：路由 + 文件操作正常工作
```

**新端点逐步替换旧端点**：
- 旧端点：`/chat/stream` → chat2.py（逐步废弃）
- 新端点：`/chat/stream/v2` → chat_router（阶段1启用）

**验证通过后**：逐步将前端请求切换到新端点，旧端点废弃

**chat_router.py 直接作为 API 端点**：
- 不需要单独的 chat_router_api.py
- chat_router.py 直接添加 FastAPI 路由装饰器
...
chat_router 在步骤6调用 react_sse_wrapper，react_sse_wrapper 内部根据 intent_type 分发到不同的处理函数：
...
步骤6: 调用到 react_sse_wrapper
        - react_sse_wrapper内部根据 intent_type 分发到不同Agent
...
async def route(self, ...):
    ...
    # react_sse_wrapper内部根据intent_type分发到不同Agent
...
## 附录6：阶段6：最终架构（构建：react_sse_wrapper）
...
【当前调用链】（简化版，3层）
chat_router.py (第一层)
  → _handle_file_operation() → FileReactAgent.ver1_run_stream() (包含 SSE 格式化)
...
chat_router.py (第一层)
  → react_sse_wrapper (第二层) - SSE包装+分发
      ↓ 根据 intent_type 分发
      ├─ → FileReactAgent.run_stream() → BaseAgent.run_stream()
      ├─ → NetworkReactAgent.run_stream() → BaseAgent.run_stream()
      ├─ → chat_stream_query() (直接流式)
      └─ → DesktopReactAgent.run_stream() → BaseAgent.run_stream()

【说明】
- BaseAgent 类存在于 base_react.py（不是 BaseReAct）
- react_sse_wrapper 负责分发，不是直接调用 file_react
...
if sse_data.startswith("data: "):
    ...
    await save_execution_steps_to_db(session_id, current_execution_steps, current_content)
```
**附录6 阶段6 实施步骤（react_sse_wrapper集成）**

> **目标**：实现 chat_router → react_sse_wrapper → 第三层Agent 的完整4层架构
> 
> **基于现有代码**：file_react.py 和 react_sse_wrapper.py

**第一步：修改 file_react.py**
- 删除 `ver1_run_stream()` 方法（约100行，第560-654行）
- 保留 `run_stream()` 方法（返回 event dict，供 react_sse_wrapper 调用）
- **保留其他所有方法**（如工具定义、Prompt模板等）
...
class FileReactAgent(BaseAgent):
    ...
    async def run_stream(self, task, context, max_steps):
        ...
        yield {"type": "observation", "obs_summary": ...}
...
| **SSE输出** | 格式化start/thought/action_tool/observation/final/error | 贯穿全文 | ✅ |
| **异常处理** | 各种异常场景的错误响应 | 269-327 | ✅ |
| **分发逻辑** | 根据intent_type分发到file/network/chat/desktop处理函数 | 新增 | ✅ |

**⚠️ 特别注意**：
- 删除 start 步骤发送逻辑（第162-186行），因为start步骤已由第一层chat_router发送
- **不要删除** 任务管理、数据库保存、异常处理等通用功能
- 每个处理函数返回 event dict，react_sse_wrapper 负责 SSE 格式化

```python
# react_sse_wrapper.py 修改
# ============================================================
# 任务管理相关（必须保留，不要删除）
# ============================================================
running_tasks_lock = asyncio.Lock()
...
async def generate_sse_stream(
    ...
    if session_id in interrupted_sessions:
        if datetime.now() - interrupted_sessions[session_id] < INTERRUPTED_SESSION_TIMEOUT:
            yield create_error_response("session_interrupted", ...)
            return
    ...
    try:
        ...
        # 【分发逻辑】根据 intent_type 分发
        if intent_type == "file" and confidence >= 0.3:
            async for event in _process_file_operation(user_input, llm_client, session_id, current_execution_steps):
                sse_data = _format_sse(event, next_step())
                # 【数据库保存】实时保存
                await save_execution_steps_to_db(session_id, current_execution_steps, current_content)
                yield sse_data
        elif intent_type == "network" and confidence >= 0.3:
            async for event in _process_network_operation(user_input, llm_client, session_id, current_execution_steps):
                sse_data = _format_sse(event, next_step())
                await save_execution_steps_to_db(session_id, current_execution_steps, current_content)
                yield sse_data
        elif intent_type == "chat" or confidence < 0.3:
            async for event in _process_chat_operation(user_input, ai_service, session_id, current_execution_steps):
                yield event  # chat_stream_query 已直接返回SSE
        elif intent_type == "desktop" and confidence >= 0.3:
            async for event in _process_desktop_operation(user_input, llm_client, session_id, current_execution_steps):
                sse_data = _format_sse(event, next_step())
                await save_execution_steps_to_db(session_id, current_execution_steps, current_content)
                yield sse_data
        else:
    ...
    # 【异常处理】必须保留
        ...
        await save_execution_steps_to_db(session_id, current_execution_steps, "错误")
...
async def _process_file_operation(user_input, llm_client, session_id, current_execution_steps):
    """file 操作处理函数"""
    from app.services.agent.file_react import FileReactAgent
    ...
    async for event in agent.run_stream(task=user_input, context=None, max_steps=100):
        current_execution_steps.append(event)  # 【数据库保存】保存到执行步骤
        yield event

async def _process_network_operation(user_input, llm_client, session_id, current_execution_steps):
    """network 操作处理函数"""
    from app.services.agent.network_react import NetworkReactAgent
    agent = NetworkReactAgent(llm_client=llm_client, session_id=session_id)
    async for event in agent.run_stream(task=user_input, context=None, max_steps=100):
        current_execution_steps.append(event)  # 【数据库保存】保存到执行步骤
        yield event

async def _process_chat_operation(user_input, ai_service, session_id, current_execution_steps):
    """chat 操作处理函数（已直接返回SSE，跳过SSE格式化）"""
    from app.chat_stream.chat_stream_query import chat_stream_query
    async for event in chat_stream_query(...):
        yield event

async def _process_desktop_operation(user_input, llm_client, session_id, current_execution_steps):
    """desktop 操作处理函数"""
    from app.services.agent.desktop_react import DesktopReactAgent
    agent = DesktopReactAgent(llm_client=llm_client, session_id=session_id)
    async for event in agent.run_stream(task=user_input, context=None, max_steps=100):
        current_execution_steps.append(event)  # 【数据库保存】保存到执行步骤
        yield event
...
# SSE 格式化函数（必须保留）
# ============================================================
def _format_sse(event: dict, step: int) -> str:
    """将 event dict 格式化为 SSE 字符串"""
    if event.get("type") == "thought":
        return format_thought_sse(step=step, content=event.get("content", ""))
...
- 删除 chat_router 内部的意图分发逻辑（`_handle_file_operation`、`_handle_chat_operation` 等）
- 步骤6改为调用 `react_sse_wrapper.generate_sse_stream()`
- **意图分发逻辑移到 react_sse_wrapper 内部**

```python
# chat_router.py 步骤6（修改前）
if intent_type == "file" and confidence >= 0.3:
...
# 改为（修改后）
async for event in react_sse_wrapper.generate_sse_stream(
    ...
    session_id=session_id,
    next_step=next_step,
    ai_service=ai_service,  # chat_stream_query 需要
):
    yield event
```

**⚠️ 注意**：删除旧的 `_handle_file_operation`、`_handle_chat_operation` 等方法
...
| 功能 | 当前端点 | 待实现 |
|------|---------|--------|
| 取消任务 | `/chat/stream/cancel/{task_id}` | 集成到 react_sse_wrapper |
| 暂停任务 | `/chat/stream/pause/{task_id}` | 集成到 react_sse_wrapper |
| 恢复任务 | `/chat/stream/resume/{task_id}` | 集成到 react_sse_wrapper |
| 用户确认 | `/chat/stream/confirm` | 集成到 react_sse_wrapper |

**实现方案**：
- react_sse_wrapper.py 已包含 running_tasks 字典
- 需要在 chat_router.py 添加对应端点
- 调用 react_sse_wrapper 的 cancel/pause/resume 函数

**实现代码示例**：
```python
# chat_router.py 添加任务控制端点

@router.post("/chat/stream/cancel/{task_id}")
async def cancel_task(task_id: str):
    """取消任务"""
    from app.services.react_sse_wrapper import cancel_task as wrapper_cancel
    result = await wrapper_cancel(task_id)
    return result

@router.post("/chat/stream/pause/{task_id}")
async def pause_task(task_id: str):
    """暂停任务"""
    from app.services.react_sse_wrapper import pause_task as wrapper_pause
    result = await wrapper_pause(task_id)
    return result

@router.post("/chat/stream/resume/{task_id}")
async def resume_task(task_id: str):
    """恢复任务"""
    from app.services.react_sse_wrapper import resume_task as wrapper_resume
    result = await wrapper_resume(task_id)
    return result

@router.post("/chat/stream/confirm")
async def confirm_operation(request: Request):
    """用户确认继续操作"""
    body = await request.json()
    task_id = body.get("task_id")
    from app.services.react_sse_wrapper import confirm_task as wrapper_confirm
    result = await wrapper_confirm(task_id, body)
    return result
```

#### 7.2 创建 network_react.py

| 项目 | 说明 |
|------|------|
| 文件名 | `network_react.py` |
| 类名 | `NetworkReactAgent` |
| 位置 | `backend/app/services/agent/network_react.py` |
| 依赖 | 继承 BaseAgent，使用 base_react.py 核心逻辑 |

**实现代码示例**：
```python
# app/services/agent/network_react.py
from app.services.agent.base_react import BaseAgent

class NetworkReactAgent(BaseAgent):
    """网络操作ReAct Agent"""
    
    def __init__(self, llm_client, session_id, intent_type="network", max_steps=100):
        super().__init__(llm_client=llm_client, session_id=session_id)
        self.intent_type = intent_type
        self.max_steps = max_steps
    
    async def run_stream(self, task, context=None):
        """返回 event dict（供 react_sse_wrapper 调用）"""
        # 实现网络操作 ReAct 循环
        # yield {"type": "thought", "content": ...}
        # yield {"type": "action_tool", "tool_name": ..., "params": ...}
        # yield {"type": "observation", "obs_summary": ...}
        # yield {"type": "final", "content": ...}
        ...
```
...
**待实现内容**：
- 网络操作工具（http请求、API调用等）
- Prompt模板
- LLM调用策略

#### 7.3 创建 desktop_react.py

| 项目 | 说明 |
|------|------|
| 文件名 | `desktop_react.py` |
| 类名 | `DesktopReactAgent` |
| 位置 | `backend/app/services/agent/desktop_react.py` |
| 依赖 | 继承 BaseAgent，使用 base_react.py 核心逻辑 |

**实现代码示例**：
```python
# app/services/agent/desktop_react.py
from app.services.agent.base_react import BaseAgent

class DesktopReactAgent(BaseAgent):
    """桌面操作ReAct Agent"""
    
    def __init__(self, llm_client, session_id, intent_type="desktop", max_steps=100):
        super().__init__(llm_client=llm_client, session_id=session_id)
        self.intent_type = intent_type
        self.max_steps = max_steps
