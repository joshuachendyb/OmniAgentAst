...
- 新增 `generate_sse_stream()` 函数，根据 intent_type 分发到不同处理函数
...
async def generate_sse_stream(
    ...
    # 根据 intent_type 分发到不同处理函数
    if intent_type == "file" and confidence >= 0.3:
        async for event in _process_file_operation(user_input, llm_client, session_id):
            yield _format_sse(event, next_step())  # SSE格式化
    elif intent_type == "network" and confidence >= 0.3:
        async for event in _process_network_operation(user_input, llm_client, session_id):
            yield _format_sse(event, next_step())
    elif intent_type == "chat" or confidence < 0.3:
        async for event in _process_chat_operation(user_input, ai_service, session_id):
            yield event  # chat_stream_query 已直接返回SSE
    elif intent_type == "desktop" and confidence >= 0.3:
        async for event in _process_desktop_operation(user_input, llm_client, session_id):
            yield _format_sse(event, next_step())
    else:
        async for event in _process_chat_operation(user_input, ai_service, session_id):
            yield event
...
# file 操作处理函数
async def _process_file_operation(user_input, llm_client, session_id):
    from app.services.agent.file_react import FileReactAgent
    ...
    async for event in agent.run_stream(task=user_input, context=None, max_steps=100):
        yield event  # 返回 event dict

# network 操作处理函数
async def _process_network_operation(user_input, llm_client, session_id):
    from app.services.agent.network_react import NetworkReactAgent
...
# chat 操作处理函数
async def _process_chat_operation(user_input, ai_service, session_id):
    from app.chat_stream.chat_stream_query import chat_stream_query
    async for event in chat_stream_query(...):  # 已直接返回SSE
        yield event

# desktop 操作处理函数
async def _process_desktop_operation(user_input, llm_client, session_id):
    from app.services.agent.desktop_react import DesktopReactAgent
