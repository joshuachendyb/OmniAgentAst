...
export const CurrentModelRefCard: React.FC<Props> = ({
  ...
  return (
    <div>
      <div style={{ fontSize: FontSize.PRIMARY, fontWeight: FontWeight.BOLD, marginBottom: Spacing.SM }}>
        当前系统全局使用模型
      </div>
      <div
        style={{
      ...
      >
        ...
        <span style={{ fontSize: FontSize.SECONDARY, color: Colors.TEXT.SECONDARY }}>
          {status === 'normal' && `当前系统全局使用模型：${providerName} / ${modelName} —— 全局默认生效`}
          {status === 'fallback' && `配置文件中 ai 块缺失或模型不可用，系统已自动使用推荐模型：${providerName} / ${modelName}。请确认或更换。`}
          {status === 'no_available' && '未配置任何可用模型，所有会话将无法调用 LLM。请先添加 Provider。'}
        </span>
        {status === 'normal' && (
      ...
      <div style={{ fontSize: FontSize.PRIMARY, fontWeight: FontWeight.BOLD, marginBottom: capabilities.length ? Spacing.SM : Spacing.MD }}>
      ...
      {capabilities.length > 0 && (
        <div style={{ marginBottom: Spacing.MD }}>
          {capabilities.map((c) => (
            <Tag key={c} style={{ fontSize: FontSize.SECONDARY, marginRight: Spacing.SM }}>
              {c}
            </Tag>
          ))}
        </div>
      ...
      <div>
        ...
        {status === 'fallback' && (
          <Button size="small" type="primary" onClick={() => setSwitchOpen(true)}>
            去选择
          </Button>
        )}
        {status === 'no_available' && (
      ...
      <ModelSwitchModal
        ...
        onOk={async (p, m) => {
          try {
            await configApi.updateConfig({ ai_model_ref: { provider: p, model: m } });
            showSuccess('模型已切换');
            setSwitchOpen(false);
            onModelSwitched();
          } catch (e) {
