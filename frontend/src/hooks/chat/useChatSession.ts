  ...
  const loadSession = useCallback(async (sid: string): Promise<Message[]> => {
    try {
      ...
      if (result) {
        ...
        setSessionTitle(result.sessionTitle || "新会话");
        setSessionVersion(result.sessionVersion || 1);
        setTitleLocked(result.titleLocked || false);
        setLastSavedTitle(result.sessionTitle || "新会话");
      ...
      if (errorResult.retryable) {
        showLoadRetryWarning(errorResult.message);
      } else {
        showLoadErrorWithKey(errorResult.message, sid);
      }
      return [];
    }
  }, [
    ...
    try {
      ...
      if (errorResult.status === 409) {
        ...
        try {
          ...
          if (result) {
            setSessionTitle(result.sessionTitle || newTitle);
            setSessionVersion(result.sessionVersion || sessionVersion + 1);
            setTitleLocked(result.titleLocked || false);
            setLastSavedTitle(result.sessionTitle || newTitle);
