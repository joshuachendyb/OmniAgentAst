...
本文档为UI UX Pro Max Skill的完整安装、使用和卸载指南。UI UX Pro Max是一个AI驱动的UI/UX设计智能技能库，提供10条行业推理规则、30种UI风格、10种配色方案、10组字体搭配，帮助开发者快速生成专业的UI/UX设计系统和代码。
...
| 功能类别 | 数量 | 说明 |
|---------|------|------|
| **行业推理规则** | 10条 | 科技、金融、医疗、电商等行业设计规则（完整版100条） |
| **UI风格** | 30种 | Glassmorphism、Neumorphism、Brutalism等（完整版57种） |
| **配色方案** | 10种 | Modern SaaS、Dark Tech、Healthcare等（完整版96种） |
| **字体搭配** | 10组 | Google Fonts最佳搭配（完整版56组） |
| UX指南 | 99条 | 最佳实践和反模式 |
...
| 类别 | 技术栈 |
|-----|-------|
| Web框架 | React、Next.js、Vue、Nuxt.js、HTML+Tailwind |
| 移动端 | Flutter、SwiftUI、React Native |
| 其他 | Svelte、Astro、Jetpack Compose |
...
export const Button = ({ children, variant = 'primary', onClick }) => {
  ...
  return (
    <button
      className={`font-medium transition-colors duration-200 ${variants[variant]}`}
      onClick={onClick}
    >
      {children}
    </button>
...
<!-- 根据设计系统自动生成的卡片组件 -->
<template>
  <div class="card glass-effect" :class="{ 'dark-mode': isDark }">
...
<style scoped>
.card {
  @apply bg-white/20 backdrop-blur-lg rounded-xl p-6 shadow-lg;
...
| v1.1 | 2026-02-05 01:58:00 | 更新：列出实际安装的30种UI风格、10种配色、10组字体；添加第五章"新增资源安装方法"，说明如何安装完整版资源包 |

---

**文档类型**: 安装指南
**更新时间**: 2026-02-05 01:58:00
**维护人**: 技术团队
