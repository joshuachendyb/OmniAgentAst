...
**理由**：
- 非递归模式：列出直接子项，数量少（通常 < 100）
- 递归模式：递归列出所有子项，数量多（可达数千）
- 逻辑简单可靠，无需设置阈值
...
递归模式下，返回的entries是扁平数组，需要前端转换为树形结构。
...
function convertEntriesToTree(entries: Entry[]): TreeNode[] {
  // 1. 创建 path -> node 的映射
  // 2. 遍历entries，构建树形结构
  // 3. 返回根节点列表
}
...
**转换步骤**：
1. **按path排序**：确保父目录在子目录之前
2. **构建路径映射**：用Map存储每个path对应的节点
3. **分配子节点**：遍历时将子节点加入父节点的children
4. **返回根节点**：path为根目录的节点

**注意**：需要确认后端返回的path是绝对路径还是相对路径
...
**更新时间**: 2026-03-23 20:20:00
**版本**: v2.3
**更新内容**: 修正行号错误；简化7.5节为方法2；新增8.1.3树形转换算法；新增8.1.4多步骤折叠状态管理
...
| v2.3 | 2026-03-23 20:20:00 | 小沈 | 简化7.5节为方法2；新增8.1.3树形转换算法；新增8.1.4多步骤折叠状态管理 |
