...
## 二、Provider流式处理体系（`llm/provider/`）— [标注:可以借鉴]
...
## 三、Agent ReAct核心循环（`llm/agent/agent.go`）— ★★（[标注:可以借鉴]用）
...
### 4.4 SubAgent模式（agent-tool.go — 对我参考价值极高）（[标注:可以借鉴]）
有几个子agent?
```go
// agent/agent-tool.go
...
## 五、Prompt架构（`llm/prompt/`）— ★（[标注:可以借鉴]用）
...
#### 5.2.1 getEnvironmentInfo() 动态注入（[标注:可以借鉴]）
