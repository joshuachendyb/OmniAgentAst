...
> **v0.15.9 重要更新**：
> - **CRSS意图关键词修复**：`txt`/`md`/`json` 从 DOCUMENT 移至 FILE，`zip`/`tar`/`gz`/`compress`/`archive` 及中文词（压缩/解压/归档/改名/复制/搜索等）加入 FILE，文件操作请求不再错分到DOC_CONTENT Agent
> - **write_text_file参数统一**：`text` → `content`，同步更新 schema/prompts/register/tools/safety 共 5 个文件
> - **JSON混合提取容错**：LLM 返回文本+JSON混合内容时，按 `tool_name` 标记向前定位 `{` 再逐字括号匹配，避免嵌套 JSON 解析失败
