...
## 二、现有代码的 ReAct Loop 的问题和解决办法

### 2.1 问题详解

#### 2.1.1 P1: conversation_history 顺序错误（严重）
...
#### 2.1.3 P3: start_request() 被重复调用（中等）
...
#### 2.1.5 P5: _on_after_loop() 未被调用（低）
...
#### 2.1.6 P6: llm_client 没有必要的方法（严重）
...
#### 2.1.8 P8: TextStrategy 返回纯文本，但 parser 期望 JSON（严重）
...
#### 2.1.10 P11: 错误处理导致LLM的content被丢弃（中等）
...
#### 2.1.11 P12: 历史消息裁剪_trim_history被注释（严重）
...
#### 2.1.12 P13: 重复添加 assistant 到 conversation_history（中等）
...
#### 2.1.13 P14: parsed_obs.content（第2次LLM响应）从未保存到conversation_history（严重）
...
#### 2.1.14 P16: parse错误后 continue 导致 SSE 事件不完整（中等）
