...
## 4.8 当前代码字段定义（2026-04-08）

> ⚠️ **说明**：以下字段定义为当前代码实际实现
...
yield {
    ...
    "reasoning": reasoning_content,  # 2026-04-08新增：LLM的reasoning字段
    "action_tool": action_tool,       # ⚠️ 旧字段名，计划统一为tool_name
    "params": params                  # ⚠️ 旧字段名，计划统一为tool_params
}
```
...
| `reasoning` | LLM的reasoning字段（可选） | ✅ 2026-04-08新增 |
| `action_tool` | 工具名称（⚠️旧名，计划改tool_name） | ⚠️ 待统一 |
| `params` | 工具参数（⚠️旧名，计划改tool_params） | ⚠️ 待统一 |
...
| **observation** | ✅ 工具原始结果 | `content=f"Tool '{action_tool}' executed: {summary}"` | ⚠️ 非原始结果 |
| **return_direct** | ✅ 支持直接返回 | 未实现 | ❌ 缺失 |

**已实现字段**：step, timestamp, tool_name, content

**关键差异**：
```
文档设计: observation="北京今天晴，25度"  ← 纯原始数据
当前实现: content="Tool 'get_weather' executed: completed"  ← 程序生成的描述
```

**可借鉴优点**：
- ⭐⭐⭐ `observation`字段：存储工具执行的**原始结果**，不是程序生成的描述
...
| 改进项 | 价值 | 复杂度 | 说明 |
|--------|------|--------|------|
| **observation改为工具原始结果** | ⭐⭐⭐⭐ | 中 | 改content为原始数据 |
| **final添加is_finished字段** | ⭐⭐⭐⭐ | 低 | 前端显示"✅ 结束"徽章 |

#### 7.6.2 中优先级（建议后续考虑）

| 改进项 | 价值 | 复杂度 | 说明 |
|--------|------|--------|------|
| **error添加recoverable字段** | ⭐⭐⭐ | 低 | 前端显示"重试"按钮 |
| **start添加task字段** | ⭐⭐ | 低 | 显示用户原始任务 |
...
| 文件 | 行号 | 修改前 | 修改后 | 修改类型 |
|------|------|--------|--------|---------|
| **base_react.py** | 174 | `"action_tool": error_info["action_tool"]` | `"tool_name": error_info["tool_name"]` | 字段名 |
| **base_react.py** | 184 | `action_tool = parsed.get("action_tool", "finish")` | `tool_name = parsed.get("tool_name", "finish")` | 变量名+字段名 |
| **base_react.py** | 194 | `"action_tool": action_tool` | `"tool_name": tool_name` | 字段名 |
| **base_react.py** | 204 | `if action_tool == "finish":` | `if tool_name == "finish":` | 变量名 |
| **base_react.py** | 208 | `params.get("result", ...)` | `tool_params.get("result", ...)` | 变量名 |
| **base_react.py** | 214 | `await self._execute_tool(action_tool, params)` | `await self._execute_tool(tool_name, tool_params)` | 变量名 |
| **base_react.py** | 222 | `"tool_name": action_tool` | `"tool_name": tool_name` | 变量名 |
| **base_react.py** | 223 | `"tool_params": params` | `"tool_params": tool_params` | 变量名 |
| **base_react.py** | 246 | `tool_name=action_tool` | `tool_name=tool_name` | 变量名 |
| **base_react.py** | 247 | `tool_params=params` | `tool_params=tool_params` | 变量名 |
| **base_react.py** | 259 | `"tool_name": action_tool` | `"tool_name": tool_name` | 变量名 |
| **base_react.py** | 260 | `f"Tool '{action_tool}' executed: ..."` | `f"Tool '{tool_name}' executed: ..."` | 变量名 |

#### 8.2.2 SSE格式化文件

| 文件 | 行号 | 修改前 | 修改后 | 修改类型 |
|------|------|--------|--------|---------|
| **sse_formatter.py** | 41 | `action_tool: str = ''` | `tool_name: str = ''` | 参数名 |
| **sse_formatter.py** | 42 | `params: Optional[Dict] = None` | `tool_params: Optional[Dict] = None` | 参数名 |
| **sse_formatter.py** | 45 | `action_tool: str = ''` | `tool_name: str = ''` | 参数名 |
| **sse_formatter.py** | 46 | `params: Optional[Dict] = None` | `tool_params: Optional[Dict] = None` | 参数名 |
| **sse_formatter.py** | 55 | `action_tool: 目标工具` | `tool_name: 目标工具` | 参数名 |
| **sse_formatter.py** | 56 | `params: 工具参数` | `tool_params: 工具参数` | 参数名 |
| **sse_formatter.py** | 64 | `'action_tool': action_tool` | `'tool_name': tool_name` | 字段名 |
| **sse_formatter.py** | 65 | `'params': params or {}` | `'tool_params': tool_params or {}` | 字段名 |

#### 8.2.3 SSE包装器文件

| 文件 | 行号 | 修改前 | 修改后 | 修改类型 |
|------|------|--------|--------|---------|
| **react_sse_wrapper.py** | 105 | `action_tool=event.get('action_tool', '')` | `tool_name=event.get('tool_name', '')` | 字段名 |
| **react_sse_wrapper.py** | 106 | `params=event.get('params', {})` | `tool_params=event.get('tool_params', {})` | 字段名 |

#### 8.2.4 LLM策略文件

| 文件 | 行号 | 修改前 | 修改后 | 修改类型 |
|------|------|--------|--------|---------|
| **llm_strategies.py** | 119 | `action_tool="finish"` | `tool_name="finish"` | 参数名 |
| **llm_strategies.py** | 120 | `params={"result": ...}` | `tool_params={"result": ...}` | 参数名 |
| **llm_strategies.py** | 126 | `action_tool="finish"` | `tool_name="finish"` | 参数名 |
| **llm_strategies.py** | 127 | `params={"result": ...}` | `tool_params={"result": ...}` | 参数名 |
| **llm_strategies.py** | 134 | `action = parsed.get("action_tool", "finish")` | `tool_name = parsed.get("tool_name", "finish")` | 变量名+字段名 |
| **llm_strategies.py** | 136 | `params = parsed.get("params", {})` | `tool_params = parsed.get("tool_params", {})` | 变量名+字段名 |
| **llm_strategies.py** | 139 | `action_tool=action, params=params` | `tool_name=tool_name, tool_params=tool_params` | 参数名 |
| **llm_strategies.py** | 146 | `tool_result['action_tool']` | `tool_result['tool_name']` | 字段名 |
| **llm_strategies.py** | 149 | `action_tool=tool_result["action_tool"]` | `tool_name=tool_result["tool_name"]` | 参数名+字段名 |
| **llm_strategies.py** | 150 | `params=tool_result.get("params", {})` | `tool_params=tool_result.get("tool_params", {})` | 参数名+字段名 |
| **llm_strategies.py** | 155 | `action_tool="finish", params={}` | `tool_name="finish", tool_params={}` | 参数名 |
| **llm_strategies.py** | 157 | `action_tool: str, params: dict` | `tool_name: str, tool_params: dict` | 参数名 |
| **llm_strategies.py** | 161 | `"action_tool": action_tool` | `"tool_name": tool_name` | 字段名 |
| **llm_strategies.py** | 162 | `"params": params` | `"tool_params": tool_params` | 字段名 |
| **llm_strategies.py** | 255 | `"action_tool": tool` | `"tool_name": tool` | 字段名 |
| **llm_strategies.py** | 257 | `"params": params` | `"tool_params": tool_params` | 字段名 |
| **llm_strategies.py** | 431 | `"action_tool": func_name` | `"tool_name": func_name` | 字段名 |
| **llm_strategies.py** | 432 | `"params": args` | `"tool_params": args` | 字段名 |
| **llm_strategies.py** | 453 | `"action_tool": first_call["name"]` | `"tool_name": first_call["name"]` | 字段名 |
| **llm_strategies.py** | 454 | `"params": first_call["args"]` | `"tool_params": first_call["args"]` | 字段名 |
| **llm_strategies.py** | 481 | `"action_tool": {"type": "string", ...}` | `"tool_name": {"type": "string", ...}` | 字段名 |
| **llm_strategies.py** | 482 | `"params": {` | `"tool_params": {` | 字段名 |
| **llm_strategies.py** | 487 | `["thought", "action_tool", "params"]` | `["thought", "tool_name", "tool_params"]` | 字段名 |
| **llm_strategies.py** | 540 | `action_tool = result.get("action_tool", "")` | `tool_name = result.get("tool_name", "")` | 变量名+字段名 |
| **llm_strategies.py** | 541 | `params = result.get("params", {})` | `tool_params = result.get("tool_params", {})` | 变量名+字段名 |
| **llm_strategies.py** | 546 | `"action_tool": action_tool` | `"tool_name": tool_name` | 字段名 |
| **llm_strategies.py** | 547 | `"params": params` | `"tool_params": tool_params` | 字段名 |
| **llm_strategies.py** | 551 | `action_tool={action_tool}` | `tool_name={tool_name}` | 变量名 |

#### 8.2.5 System Prompt文件

| 文件 | 行号 | 修改前 | 修改后 | 修改类型 |
|------|------|--------|--------|---------|
| **file_prompts.py** | 141 | `"action_tool": "list_directory"` | `"tool_name": "list_directory"` | 字段名 |
| **file_prompts.py** | 142 | `"params": {` | `"tool_params": {` | 字段名 |
| **file_prompts.py** | 151 | `"action_tool": "read_file"` | `"tool_name": "read_file"` | 字段名 |
| **file_prompts.py** | 152 | `"params": {` | `"tool_params": {` | 字段名 |
| **file_prompts.py** | 161 | `"action_tool": "search_file_content"` | `"tool_name": "search_file_content"` | 字段名 |
| **file_prompts.py** | 162 | `"params": {` | `"tool_params": {` | 字段名 |
| **file_prompts.py** | 172 | `"action_tool": "move_file"` | `"tool_name": "move_file"` | 字段名 |
| **file_prompts.py** | 173 | `"params": {` | `"tool_params": {` | 字段名 |
| **file_prompts.py** | 202 | `"action_tool": "tool_name"` | `"tool_name": "tool_name"` | 字段名 |
| **file_prompts.py** | 203 | `"params": {` | `"tool_params": {` | 字段名 |
| **file_prompts.py** | 209 | `"action_tool": "finish", "params": {...}` | `"tool_name": "finish", "tool_params": {...}` | 字段名 |
| **file_prompts.py** | 232 | `action_tool="finish"` | `tool_name="finish"` | 字段名 |
| **file_prompts.py** | 234 | `"action_tool": "finish", "params": {...}` | `"tool_name": "finish", "tool_params": {...}` | 字段名 |

#### 8.2.6 类型定义文件

| 文件 | 行号 | 修改前 | 修改后 | 修改类型 |
|------|------|--------|--------|---------|
| **step_types.py** | 23 | `action_tool: str = ""` | `tool_name: str = ""` | 字段名 |
| **step_types.py** | 24 | `params: Dict[str, Any]` | `tool_params: Dict[str, Any]` | 字段名 |
| **step_types.py** | 31 | `"action_tool": self.action_tool` | `"tool_name": self.tool_name` | 字段名 |
| **step_types.py** | 32 | `"params": self.params` | `"tool_params": self.tool_params` | 字段名 |
| **step_types.py** | 68 | `action_tool: str = ""` | `tool_name: str = ""` | 字段名 |
| **step_types.py** | 69 | `params: Dict[str, Any]` | `tool_params: Dict[str, Any]` | 字段名 |
| **step_types.py** | 80 | `"action_tool": self.action_tool` | `"tool_name": self.tool_name` | 字段名 |
| **step_types.py** | 81 | `"params": self.params` | `"tool_params": self.tool_params` | 字段名 |

#### 8.2.7 工具解析器文件

| 文件 | 行号 | 修改前 | 修改后 | 修改类型 |
|------|------|--------|--------|---------|
| **tool_parser.py** | 57 | `if "params" in parsed:` | `if "tool_params" in parsed:` | 字段名 |
| **tool_parser.py** | 58 | `params = parsed.get("params", {})` | `tool_params = parsed.get("tool_params", {})` | 字段名 |
| **tool_parser.py** | 60 | `params = parsed.get("action_input", {})` | `tool_params = parsed.get("action_input", {})` | 变量名 |
| **tool_parser.py** | 62 | `params = parsed.get("actionInput", {})` | `tool_params = parsed.get("actionInput", {})` | 变量名 |
| **tool_parser.py** | 64 | `params = {}` | `tool_params = {}` | 变量名 |
| **tool_parser.py** | 71 | `"params": params` | `"tool_params": tool_params` | 字段名 |
| **tool_parser.py** | 256 | `"params": {}` | `"tool_params": {}` | 字段名 |

#### 8.2.8 工具文件

| 文件 | 行号 | 修改前 | 修改后 | 修改类型 |
|------|------|--------|--------|---------|
| **quick_test_longcat.py** | 23 | `"action_tool": {"type": "string"}` | `"tool_name": {"type": "string"}` | 字段名 |
| **quick_test_longcat.py** | 26 | `["thought", "action_tool", "params"]` | `["thought", "tool_name", "tool_params"]` | 字段名 |
| **test_longcat_structured_outputs.py** | 38 | `"action_tool": {"type": "string", ...}` | `"tool_name": {"type": "string", ...}` | 字段名 |
| **test_longcat_structured_outputs.py** | 47 | `["thought", "action_tool", "params"]` | `["thought", "tool_name", "tool_params"]` | 字段名 |
| **test_file_operation_direct.py** | 57 | `event.get('action_tool')` | `event.get('tool_name')` | 字段名 |

---

### 8.3 测试文件修改清单

#### 8.3.1 test_react_refactor.py

| 行号 | 修改前 | 修改后 | 修改类型 |
|------|--------|--------|---------|
| 6 | `action_tool, params` | `tool_name, tool_params` | 注释 |
| 7 | `action_tool, params` | `tool_name, tool_params` | 注释 |
| 133 | `action_tool, params` | `tool_name, tool_params` | 注释 |
| 150 | `test_parse_response_returns_action_tool` | `test_parse_response_returns_tool_name` | 函数名 |
| 151 | `返回新字段action_tool` | `返回新字段tool_name` | 注释 |
| 154 | `"action_tool": "list_directory"` | `"tool_name": "list_directory"` | 字段名 |
| 160 | `assert "action_tool" in result` | `assert "tool_name" in result` | 字段名 |
| 161 | `result["action_tool"]` | `result["tool_name"]` | 字段名 |
| 176 | `result["action_tool"]` | `result["tool_name"]` | 字段名 |
| 187 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 214 | `"action_tool": "list_directory"` | `"tool_name": "list_directory"` | 字段名 |
| 232 | `assert "action_tool" in thought_event` | `assert "tool_name" in thought_event` | 字段名 |
| 237 | `action_tool阶段` | `tool_name阶段` | 注释 |
| 355 | `action_tool + params` | `tool_name + tool_params` | 注释 |
| 356 | `action_tool` | `tool_name` | 注释 |
| 366 | `action_tool="list_directory"` | `tool_name="list_directory"` | 参数名 |
| 370 | `thought.action_tool` | `thought.tool_name` | 字段名 |
| 391 | `action_tool="finish"` | `tool_name="finish"` | 参数名 |
| 397 | `observation.action_tool` | `observation.tool_name` | 字段名 |
| 404 | `action_tool, params` | `tool_name, tool_params` | 注释 |
| 417 | `action_tool="finish"` | `tool_name="finish"` | 参数名 |
| 426 | `observation.action_tool` | `observation.tool_name` | 字段名 |
| 441 | `"action_tool": "list_directory"` | `"tool_name": "list_directory"` | 字段名 |
| 450 | `test_action_tool_fields_not_nested` | `test_tool_name_fields_not_nested` | 函数名 |
| 451 | `action_tool结果` | `tool_name结果` | 注释 |
| 452 | `action_tool` | `tool_name` | 注释 |
| 453 | `action_tool/params` | `tool_name/tool_params` | 注释 |
| 456 | `"type": "action_tool"` | `"type": "action_tool"` | 不改（type值） |
| 466 | `action_data.get("tool_name")` | `action_data.get("tool_name")` | 不改 |
| 479 | `"action_tool": "list_directory"` | `"tool_name": "list_directory"` | 字段名 |
| 485 | `assert "action_tool" in event` | `assert "tool_name" in event` | 字段名 |
| 514 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 522 | `assert "action_tool" in event` | `assert "tool_name" in event` | 字段名 |
| 577 | `obs_action_tool` | `obs_tool_name` | 字段名 |
| 591 | `assert "obs_action_tool" not in event` | `assert "obs_tool_name" not in event` | 字段名 |
| 597 | `action_tool = "list_directory"` | `tool_name = "list_directory"` | 变量名 |
| 601 | `f"Tool '{action_tool}' executed: ..."` | `f"Tool '{tool_name}' executed: ..."` | 变量名 |
| 631 | `finish在action_tool阶段检测` | `finish在tool_name阶段检测` | 注释 |
| 636 | `action_tool执行后` | `tool_name执行后` | 注释 |
| 644 | `parsed_obs.get("action_tool")` | `parsed_obs.get("tool_name")` | 字段名 |
| 645 | `action_tool阶段的action_tool值` | `tool_name阶段的tool_name值` | 注释 |
| 653 | `test_final_only_when_action_tool_is_finish` | `test_final_only_when_tool_name_is_finish` | 函数名 |
| 654 | `action_tool是finish` | `tool_name是finish` | 注释 |
| 656 | `action_tool阶段` | `tool_name阶段` | 注释 |
| 657 | `action_tool` | `tool_name` | 注释 |
| 660 | `action_tool = "finish"` | `tool_name = "finish"` | 变量名 |
| 661 | `action_tool` | `tool_name` | 变量名 |
| 663 | `if action_tool == "finish":` | `if tool_name == "finish":` | 变量名 |
| 672 | `action_tool = "list_directory"` | `tool_name = "list_directory"` | 变量名 |
| 673 | `action_tool` | `tool_name` | 变量名 |
| 675 | `if action_tool == "finish":` | `if tool_name == "finish":` | 变量名 |

#### 8.3.2 test_agent.py

| 行号 | 修改前 | 修改后 | 修改类型 |
|------|--------|--------|---------|
| 8 | `action_tool → observation` | `tool_name → observation` | 注释 |
| 41 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 61 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 76 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 92 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 110 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 126 | `events[0]["action_tool"]` | `events[0]["tool_name"]` | 字段名 |
| 132 | `action_tool → observation` | `tool_name → observation` | 注释 |
| 134 | `"action_tool": "read_file"` | `"tool_name": "read_file"` | 字段名 |
| 135 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 155 | `assert "action_tool" in event_types` | `assert "tool_name" in event_types` | 字段名 |
| 164 | `"action_tool": "read_file"` | `"tool_name": "read_file"` | 字段名 |
| 193 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 212 | `assert "action_tool" in thought_event` | `assert "tool_name" in thought_event` | 字段名 |
| 216 | `test_action_tool_event_structure` | `test_tool_name_event_structure` | 函数名 |
| 217 | `action_tool 事件结构` | `tool_name 事件结构` | 注释 |
| 219 | `"action_tool": "read_file"` | `"tool_name": "read_file"` | 字段名 |
| 220 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 237 | `action_events` | `tool_name_events` | 变量名 |
| 252 | `"action_tool": "read_file"` | `"tool_name": "read_file"` | 字段名 |
| 253 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 276 | `assert "obs_action_tool" in obs_event` | `assert "obs_tool_name" in obs_event` | 字段名 |
| 287 | `"action_tool": "read_file"` | `"tool_name": "read_file"` | 字段名 |
| 288 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 309 | `action_events` | `tool_name_events` | 变量名 |
| 328 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 348 | `"action_tool": "read_file"` | `"tool_name": "read_file"` | 字段名 |
| 349 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 372 | `"action_tool": "read_file"` | `"tool_name": "read_file"` | 字段名 |
| 373 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 400 | `"action_tool": "read_file"` | `"tool_name": "read_file"` | 字段名 |
| 401 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 427 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 439 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |
| 463 | `"action_tool": "finish"` | `"tool_name": "finish"` | 字段名 |

#### 8.3.3 test_function_calling_comprehensive.py

| 行号 | 修改前 | 修改后 | 修改类型 |
|------|--------|--------|---------|
| 795 | `result["action_tool"]` | `result["tool_name"]` | 字段名 |
| 865 | `result["action_tool"]` | `result["tool_name"]` | 字段名 |

#### 8.3.4 test_agent_llm_adapter.py

| 行号 | 修改前 | 修改后 | 修改类型 |
|------|--------|--------|---------|
| 482 | `assert "action_tool" in parsed` | `assert "tool_name" in parsed` | 字段名 |
| 708 | `test_response_format_converts_action_to_action_tool` | `test_response_format_converts_action_to_tool_name` | 函数名 |
| 709 | `action 转换为 action_tool` | `action 转换为 tool_name` | 注释 |
| 731 | `assert "action_tool" in parsed` | `assert "tool_name" in parsed` | 字段名 |
| 732 | `parsed["action_tool"]` | `parsed["tool_name"]` | 字段名 |
...
1. **type字段值不改**：`"type": "action_tool"`保持不变，这是前端识别事件类型的值
2. **变量名统一**：所有`action_tool`变量改为`tool_name`，所有`params`变量改为`tool_params`
3. **字段名统一**：所有JSON字段名统一为`tool_name`和`tool_params`
4. **函数参数名统一**：所有函数参数名统一为`tool_name`和`tool_params`
5. **注释同步更新**：所有相关注释同步更新
...
1. **第一步**：修改`step_types.py`（类型定义）
2. **第二步**：修改`base_react.py`（核心逻辑）
3. **第三步**：修改`llm_strategies.py`（LLM策略）
4. **第四步**：修改`file_prompts.py`（System Prompt）
5. **第五步**：修改`sse_formatter.py`和`react_sse_wrapper.py`（SSE相关）
6. **第六步**：修改`tool_parser.py`（工具解析器）
7. **第七步**：修改测试文件
8. **第八步**：修改工具文件

---

**更新时间**: 2026-04-08 16:55:04  
**版本**: v4.23  
**编写人**: 小资  
**更新说明**: 审查并修正第9章，修正sse.ts修改思路，明确MessageItem.tsx修改范围

---

## 九、前端tool_name/tool_params统一化修改方案

**分析时间**: 2026-04-08 16:55:04  
**分析人**: 小资（审查）/ 小强（初稿）  
**目标**: 统一前端系统字段名为`tool_name`和`tool_params`
...
1. **字段改名**：将`sse.ts`第81-82行的字段名从`action_tool`改为`tool_name`，`params`改为`tool_params`
2. **保留字段**：第97-98行的`tool_name`/`tool_params`保持不变
3. **统一访问**：所有渲染和导出逻辑使用`tool_name`/`tool_params`
4. **type值不变**：`step.type === "action_tool"`保持不变（这是步骤类型标识）
...
| 行号 | 修改前 | 修改后 | 修改类型 |
|------|--------|--------|---------|
| 81 | `action_tool?: string` | `tool_name?: string` | 字段改名 |
| 82 | `params?: Record<string, any>` | `tool_params?: Record<string, any>` | 字段改名 |

**说明**：将thought类型的字段名改为与action_tool类型一致，统一为`tool_name`和`tool_params`。
...
| 行号 | 修改前 | 修改后 | 修改类型 |
|------|--------|--------|---------|
| 433 | `{(step as any).action_tool &&` | `{(step as any).tool_name &&` | 字段名 |
| 435 | `⬇️ 下一步：{(step as any).action_tool}` | `⬇️ 下一步：{(step as any).tool_name}` | 字段名 |
| 439 | `{(step as any).params &&` | `{(step as any).tool_params &&` | 字段名 |
| 447 | `data={(step as any).params}` | `data={(step as any).tool_params}` | 字段名 |
| 721 | `action_tool: step.action_tool, params: step.params` | `tool_name: step.tool_name, tool_params: step.tool_params` | 字段名 |

**修改详情**：
- **第433-437行**：thought类型的渲染，显示"下一步"操作
- **第439-449行**：thought类型的参数显示
- **第721行**：JSON导出逻辑
...
| 行号 | 代码 | 说明 |
|------|------|------|
| 217 | `🔧 {step.tool_name \|\| "执行中..."}` | action_tool类型渲染，已用新字段 |
| 218 | `{step.tool_params &&` | action_tool类型渲染，已用新字段 |
| 723 | `tool_name: step.tool_name, tool_params: step.tool_params` | action_tool类型导出，已用新字段 |

---

#### 9.3.3 动态状态显示

**文件**: `frontend/src/utils/dynamicStatus.tsx`

| 行号 | 当前内容 | 修改建议 | 修改类型 |
|------|---------|---------|---------|
| 18 | `text: 'Agent 正在执行"action_tool"'` | `text: 'Agent 正在执行"tool_name"'` | 可选修改 |

**说明**：第18行是thought步骤的动态状态显示文本，将`action_tool`改为`tool_name`以保持一致性。第19行的`action_tool:`是配置key，不需要修改。

---

#### 9.3.4 测试文件

##### 9.3.4.1 ReAct-stream-messages.test.tsx

| 行号 | 修改前 | 修改后 | 修改类型 |
|------|--------|--------|---------|
| 58 | `action_tool: 'search_flight'` | `tool_name: 'search_flight'` | 字段改名 |
| 96 | `action_tool: 'list_directory'` | `tool_name: 'list_directory'` | 字段改名 |
| 220-221 | `expect(TEST_MESSAGES.thought.action_tool)` | `expect(TEST_MESSAGES.thought.tool_name)` | 期望值 |
| 225-226 | `expect(TEST_MESSAGES.thought.params)` | `expect(TEST_MESSAGES.thought.tool_params)` | 期望值 |
| 616-623 | `thoughtWithoutAction.action_tool` | `thoughtWithoutAction.tool_name` | 测试用例 |
| 其他 | 所有引用 `thought.action_tool` / `thought.params` | `thought.tool_name` / `thought.tool_params` | 全面更新 |

---

##### 9.3.4.2 MessageItem-fields.test.tsx

| 行号 | 修改前 | 修改后 | 修改类型 |
|------|--------|--------|---------|
| 163 | `action_tool: 'list_directory'` | `tool_name: 'list_directory'` | 测试数据 |

**说明**：注释中的"action_tool阶段"保持不变（这是type值描述）。

---

##### 9.3.4.3 dynamicStatus.test.tsx

| 行号 | 修改前 | 修改后 | 修改类型 |
|------|--------|--------|---------|
| 45 | `应该显示"Agent 正在执行action_tool"` | `应该显示"Agent 正在执行tool_name"` | 期望文本 |
| 51 | `expect(container.textContent).toContain('action_tool')` | `expect(container.textContent).toContain('tool_name')` | 断言 |
...
| 文件 | 修改内容 | 说明 |
|------|---------|------|
| StepRow-visual.test.tsx | 无修改 | type值，保持不变 |
| frontend-paging.test.tsx | 无修改 | type值，保持不变 |
| MessageItem.test.tsx | 更新测试数据字段 | `action_tool` → `tool_name` |
| ExecutionPanel.test.tsx | 无修改 | type值，保持不变 |
...
### 9.5 不需要修改的位置（type字段值）

⚠️ **以下位置的 `action_tool` 是type字段值，识别步骤类型，必须保持不变**：

| 文件 | 行号 | 代码 | 说明 |
|------|------|------|------|
| MessageItem.tsx | 214 | `step.type === "action_tool" &&` | 类型判断 |
| MessageItem.tsx | 722 | `case 'action_tool':` | switch分支 |
| MessageItem.tsx | 1226 | `step.type === 'action_tool'` | 类型判断 |
| dynamicStatus.tsx | 19 | `action_tool: { icon: '👁️'...` | 状态配置key |
| ExecutionPanel.tsx | 318 | `case "action_tool":` | switch分支 |
| 多个测试文件 | - | `type: 'action_tool'` | 测试数据type值 |
...
- [ ] sse.ts 第81-82行已改为 `tool_name` 和 `tool_params`
- [ ] MessageItem.tsx thought类型渲染已使用 `tool_name`/`tool_params`
- [ ] MessageItem.tsx 导出逻辑已使用 `tool_name`/`tool_params`
- [ ] dynamicStatus.tsx 显示文本已更新（可选）
- [ ] 所有测试文件中的字段引用已更新
- [ ] type="action_tool" 判断逻辑保持不变
- [ ] 运行 `npm run lint` 无错误
- [ ] 运行 `npm test` 所有测试通过
...
1. **区分type值和字段名**：
   - `step.type === "action_tool"` 是type值，判断步骤类型，**不能改**
   - `step.action_tool` 是数据字段，存储工具名称，**需要改为tool_name**

2. **测试数据中的type值**：
   - `type: 'action_tool' as const` 是测试数据的type字段，**不能改**
   - `action_tool: 'list_directory'` 是数据字段，**需要改为tool_name**

3. **修改顺序建议**：
   - 第一步：修改 `sse.ts` 类型定义
   - 第二步：修改 `MessageItem.tsx` 渲染和导出
   - 第三步：修改测试文件
   - 第四步：运行测试验证
...
**初稿编写人**: 小强  
**初稿时间**: 2026-04-08 16:39:54  
**审查修正人**: 小资  
**审查时间**: 2026-04-08 16:55:04
