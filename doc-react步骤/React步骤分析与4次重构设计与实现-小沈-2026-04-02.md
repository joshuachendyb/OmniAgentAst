...
```python
{
    ...
    "reasoning": reasoning,       ⚠️ 设计文档无此字段
    "action_tool": action_tool,   ✅ (文档用 tool_name)
    "params": params              ✅ (文档用 tool_params)
}
```

**前端使用** (MessageItem.tsx 第425行):
```typescript
{step.reasoning || step.content || ""}
```

**分析结论**: ❌ reasoning字段是多余的，可以删除
- 设计文档定义：thought.content = LLM的推理过程（纯自然语言）
- reasoning 只是LLM返回的一个特定字段（某些模型如Claude特有）
- content 才是完整内容，两者实际存储同类信息
- 前端的 `step.reasoning || step.content` 只是向后兼容写法

**决策**: ❌ 删除 reasoning 字段，统一用 content
...
**后端实现** (base_react.py 219-230行):
```python
{
    "type": "action_tool",
    "step": step_count,           ✅
    "timestamp": current_time,   ✅
    "content": action_tool,       ⚠️ 设计文档无此字段
    "tool_name": action_tool,     ✅
    "tool_params": params,         ✅
    "execution_status": ...,      ✅
    "summary": ...,               ✅
    "raw_data": ...,              ✅
    "action_retry_count": ...     ✅ (文档用 retry_count)
}
```
...
**分析结论**: ⚠️ content字段被前端覆盖，无实际用途
- 后端传 content = tool_name（工具名）
- 前端用 action_description 覆盖显示
- 后端的content字段实际没用到

**决策**: ❌ 删除后端的content字段（保留tool_name即可）
...
**决策**: 
- ✅ 字段一致
- ✅ return_direct 可选字段暂时不添加，后续看需求再定
...
**后端实现** (base_react.py 207-211行):
```python
{
    "type": "final",
    "timestamp": current_time,   ✅
    "content": ...,              ⚠️ 文档用 response
    # ✅ step字段已在sse_wrapper层自动添加
    # ❌ 缺少 is_finished
}
```

**外层函数补充** (react_sse_wrapper.py 第125-132行):
- react_sse_wrapper.py调用create_final_response时传入step
- chat_helpers.py第94-95行会添加step字段

**分析结论**:
- step字段：✅ 已有（在外层函数react_sse_wrapper自动添加）
- is_finished字段：不需要，action_tool == "finish"就是完成标志

**决策**: 
- ❌ 不需要添加 step（已在sse_wrapper层自动添加）
- ❌ 不需要 is_finished 字段
...
**后端实现** (base_react.py 269-283行):
```python
{
    "type": "error",
    "timestamp": create_timestamp(), ✅
    "code": "MAX_STEPS_EXCEEDED",     ⚠️ 文档用 error_type
    "message": "...",               ✅ (文档用 error_message)
    # ❌ 缺少 step
    # ❌ 缺少 error_type
    # ❌ 缺少 recoverable
}
...
**外层函数补充** (react_sse_wrapper.py 第133-142行):
- react_sse_wrapper.py调用create_error_response时传入step和error_type
- error_handler.py第97-98行会添加step字段，第95行会添加error_type字段

**分析结论**:
- step字段：✅ 已有（在外层函数react_sse_wrapper自动添加）
- error_type字段：✅ 已有（在外层函数react_sse_wrapper自动添加）
- recoverable字段：不需要，用execution_status就能判断

**决策**: 
- ❌ 不需要添加 step（已在sse_wrapper层自动添加）
- ❌ 不需要添加 error_type（已在sse_wrapper层自动添加）
- ❌ 不需要 recoverable 字段
...
| 类型 | 字段 | 决策 | 说明 |
|------|------|------|------|
| start | session_id | ✅ 添加 | 有意义，可追溯会话 |
| thought | reasoning | ❌ 删除 | 设计文档无此字段，与content重复 |
| action_tool | content | ❌ 删除 | 被前端覆盖，无实际用途 |
| observation | return_direct | ⚠️ 暂不添加 | 可选字段，后续再看需求 |
| final | step | ❌ 不需要 | 已在sse_wrapper层自动添加 |
| final | is_finished | ❌ 不需要 | action_tool=="finish"就是完成标志 |
| error | step | ❌ 不需要 | 已在sse_wrapper层自动添加 |
| error | error_type | ❌ 不需要 | 已在sse_wrapper层自动添加 |
| error | recoverable | ❌ 不需要 | execution_status已能判断 |
...
**后端修改**:
1. `backend/app/chat_stream/start_step.py` - 添加session_id字段
2. `backend/app/services/agent/base_react.py` - 删除action_tool的content，删除thought的reasoning

**前端修改**:
1. `frontend/src/utils/sse.ts` - 删除解析reasoning字段
2. `frontend/src/components/Chat/MessageItem.tsx` - 删除reasoning相关内容

**修改时间**: 2026-04-08 10:00:00  
**编写人**: 小强
