...
USER_INPUT = (
    "帮我在E:\\test_dir下创建一个e2e_mc01.txt，里面写入测试数据，然后读出来看看写对了没有，再搜一下里面有没有测试这个词，接着在文件末尾追加一行数据，追加完再读一遍确认内容完整了，然后帮我查一下这个文件的大小和修改日期，最后列出E:\\test_dir下的所有文件确认创建成功。把本次任务的分析实施过程和分析结果独立生成四种版本的报告存入report目录下。"
)
