"""全链路E2E集成测试 - P0-09: SHELL+SYSTEM+NETWORK混合通路验证
...
@pytest.mark.asyncio
async def test_e2e_p0_09_shell_system_network():
    """P0-09: SHELL+SYSTEM+NETWORK混合通路"""
    ...
    try:
        register_pending_record(
            "E2E-P0-09", "SHELL+SYSTEM+NETWORK混合通路(09)",
        ...
        assert len(result["tool_calls"]) > 0, "必须调用工具(MUST P0-09)"
        ...
        safety_errors = [e for e in lc["errors"] if any(k in e for k in ["安全检查", "拒绝执行", "高风险", "execute_code", "pickle", "RCE"])]
        ...
        print_report(
            "E2E-P0-09", "SHELL+SYSTEM+NETWORK混合通路", result, db, lc,
            ci, si, True, elapsed,
        ...
        write_test_record("E2E-P0-09", "SHELL+SYSTEM+NETWORK混合通路(09)", user_input, r, db, ci, si, lc, passed, r.get("total_time_ms", 0)/1000.0 if r else 0, dpi=dpi, error_info=error_info)

    if passed:
        print(f"\n  [DONE] E2E-P0-09 PASSED")
