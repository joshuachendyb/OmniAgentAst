...
操作手册对照:
   用例: E2E-P0-02
   用户输入: "在E盘根目录创建一个e2e_p0_02.txt，内容为hello，然后读出来确认写对了"
   前置数据: E盘根目录可写
...
USER_INPUT = "请先在E盘根目录创建一个名为e2e_p0_02.txt的文件，文件内容写入'hello world 测试文件'。然后读取该文件并验证内容是否正确写入。验证通过后，在文件末尾追加一行:第二行内容已追加，证明追加功能正常，再次读取确认修改生效了。最后把文件的完整内容以及整个操作过程中你的每一步操作告诉我。"
