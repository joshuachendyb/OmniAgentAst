...
USER_INPUT = (
    "帮我看看现在几点了，把当前日期和具体时间都查出来，包括今天是星期几和今年的第几天，然后用当前时间做文件名（比如20260615_1430.txt）在E:\\test_dir下创建个文件，里面写上详细的时间信息包括年月日时分秒和星期几，写完之后读出来看看内容对不对，再帮我查看一下这个文件的大小和创建时间。把本次任务的分析实施过程和分析结果独立生成四种版本的报告存入report目录下。"
)
