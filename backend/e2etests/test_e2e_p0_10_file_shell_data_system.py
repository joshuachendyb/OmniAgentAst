"""全链路E2E集成测试 - P0-10: FILE+SHELL+DATA+SYSTEM混合通路验证
...
@pytest.mark.asyncio
async def test_e2e_p0_10_file_shell_data_system():
    """P0-10: FILE+SHELL+DATA+SYSTEM混合通路"""
    ...
    try:
        register_pending_record(
            "E2E-P0-10", "FILE+SHELL+DATA+SYSTEM混合通路(10)",
        ...
        assert len(result["tool_calls"]) > 0, "必须调用工具(MUST P0-10)"
        ...
        print_report(
            "E2E-P0-10", "FILE+SHELL+DATA+SYSTEM混合通路", result, db, lc,
            ci, si, True, elapsed,
        ...
        write_test_record("E2E-P0-10", "FILE+SHELL+DATA+SYSTEM混合通路(10)", user_input, r, db, ci, si, lc, passed, r.get("total_time_ms", 0)/1000.0 if r else 0, dpi=dpi, error_info=error_info)

    if passed:
        print(f"\n  [DONE] E2E-P0-10 PASSED")
