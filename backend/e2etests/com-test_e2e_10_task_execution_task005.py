"""全链路E2E集成测试 - unit-10: 任务书执行验证(task005)
严禁在脚本内设任何超时 — 统一由pytest.ini的timeout=3000管理

操作手册对照:
    用例: unit-10
     用户输入: "读取任务书E:\\test_dir\\task\\task005.txt,分析可行性和实施方案,逐一执行全部任务,做任务总结"
      前置数据: E:\test_dir\task\task005.txt存在
...
-- 小欧 2026-07-17 (基于 unit-06 复制; 与 case-06 互换读取目标: 本用例读取 task005.txt)
...
TASK_FILE = Path("E:/test_dir/task/task005.txt")  # 2026-08-12 小欧: 修复错误路径(原task001.txt与user_input的task005.txt不一致,与08/12同类漏改,现断言真正校验task005)


@pytest.mark.e2e_full_link
@pytest.mark.asyncio
async def test_e2e_unit_10_task_execution():
    """unit-10: 任务书执行 - 读取任务书(task005.txt)逐一执行全部任务"""
    ...
    error_info = None
    user_input = (
        '你需要读取任务书"E:\test_dir\task\task005.txt"的要求,'
        "分析任务的可行性和实施方案,"
    ...
    try:
        ...
        # unit-08核心: 必须调用readtext读取任务书
        tool_names = [t["tool_name"] for t in result["tool_calls"]]
        file_tools = {"readtext"}
        has_read = any(n in file_tools for n in tool_names)
        assert has_read, f"必须调用readtext(MUST unit-08), 实际: {tool_names}"

        # unit-08核心: 回复应包含任务总结关键词
        ...
        assert has_task_keyword, f"回复应包含任务总结关键词(MUST unit-08), 回复: {resp[:200]}"
        ...
        assert db["execution_steps_count"] >= 1, (
            f"execution_steps必须有记录(MUST unit-08), 实际={db['execution_steps_count']}"
        )
        ...
        assert len(_ti) == 0, f"DB steps中应有readtext操作(MUST unit-08): {_ti}"
