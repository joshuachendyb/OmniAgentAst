...
USER_INPUT = (
    "帮我设置一个系统资源监控任务来持续观察电脑的运行状态。"
    "先获取当前的系统时间作为监控的基准时间点。"
    "然后查看一下当前的CPU使用率和内存使用率，记录下这组数据作为第一次采样。"
    "等5秒钟后再获取一次CPU使用率和内存使用率，作为第二次采样数据。"
    "如果在任何一次采样中发现CPU使用率超过了80%或者内存使用率超过90%，"
    "就立即发送一条系统告警通知到通知栏，告知资源使用超标的情况。"
    "最后将两次采样的所有监控数据——每个时间点的具体时间、对应的CPU使用率、"
    "内存使用率、以及是否触发了告警——整理成一份监控报告，"
    "保存到E:\\test_dir\\monitor_report.txt文件中。"
