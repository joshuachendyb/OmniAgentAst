...
USER_INPUT = (
    "帮我做一套完整的操作：先用AI行业热点和人工智能新闻今天两个关键词分别搜索今天AI行业的最新消息，把每个搜索的前几条结果的详细内容都打开看看，从里面提取出最重要的新闻要点汇总成结构化的新闻摘要，保存到E:\\test_dir\\ai_news.txt，保存完之后记录一下当前时间作为完成时间，然后给我发一条通知告诉我所有操作已完成。把本次任务的分析实施过程和分析结果独立生成四种版本的报告存入report目录下。"
)
