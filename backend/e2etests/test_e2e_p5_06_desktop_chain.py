...
USER_INPUT = (
    "帮我先列出当前所有打开的窗口看看都有什么，然后截个当前屏幕的图保存到E:\\test_dir\\screenshot.png，再看看剪贴板里有没有什么内容，查一下当前屏幕的分辨率是多少，然后用OCR识别一下截图里有哪些文字，把识别出来的文字内容保存到E:\\test_dir。把本次任务的分析实施过程和分析结果独立生成四种版本的报告存入report目录下。"
)
