...
操作手册对照:
   用例: E2E-P0-00 (新增)
    用户输入: "多维度系统介绍自身(文件/代码/网络/系统/数据),技术原理,能力边界对比表;纯对话禁工具"
   前置数据: 无
...
USER_INPUT = ("嗨，听说你是一个AI助手？我很好奇你到底是什么样的存在。请围绕以下维度系统介绍你自己："
               "(1)核心能力边界——文件操作、写代码与执行、上网搜索、系统管理、数据分析分别能到什么程度，每类举2个具体例子；"
               "(2)背后的技术架构与模型工作原理，用通俗语言解释流式输出是怎么实现的；"
               "(3)你能处理哪些文件类型与数据格式；"
               "(4)哪些场景你会主动拒绝或要求我确认。你可以把它理解为一次完整的自我画像。最后用一张对比表总结"
               "'你能做 / 不能做 / 需要确认'三类。注意：本次只用对话回答，不要调用任何工具。")
