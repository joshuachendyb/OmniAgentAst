...
USER_INPUT = (
    "帮我把这台电脑的详细系统信息查出来，查一下CPU型号和使用率、内存大小和已用空间、磁盘分区和各个分区的使用情况，再看看操作系统的版本和系统已经运行了多长时间，把PATH环境变量和系统环境信息也查出来，把所有信息汇总整理成一份完整的系统档案报告保存到E:\\test_dir。把本次任务的分析实施过程和分析结果独立生成四种版本的报告存入report目录下。"
)
