...
USER_INPUT = (
    "帮我全面诊断一下当前的Python开发环境配置是否正常。"
    "首先查看系统的PATH环境变量都设置了哪些路径，确认Python的安装目录是否已经包含在PATH中。"
    "然后查找python.exe命令的完整路径，如果找到了就获取它的版本号和架构信息。"
    "再查看PYTHONPATH这个环境变量有没有设置、如果设置了是哪些路径。"
    "接下来读取Windows注册表中Python的安装信息——包括安装路径、版本号以及注册的组件。"
    "最后把全部诊断结果汇总成一份环境诊断报告，"
    "保存到E:\\test_dir\\env_diagnose.txt文件中，"
    "报告要包含PATH内容、Python路径和版本、PYTHONPATH信息、注册表信息，"
    "以及对环境配置是否完善的整体评价。"
