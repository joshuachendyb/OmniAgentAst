...
TEST_CASE_NAME = "综合报告"
USER_INPUT = (
    "帮我收集以下几样信息：先是当前时间和日期，然后查一下系统CPU使用率、内存占用情况和各磁盘分区的剩余空间，再列一下E:\\test_dir下所有文件的详细信息包括每个文件的大小和修改日期，查一下电脑的操作系统版本和计算机名称，把以上所有信息汇总整理成一份格式化的综合报告保存到E:\\test_dir。把本次任务的分析实施过程和分析结果独立生成四种版本的报告存入report目录下。"
)
