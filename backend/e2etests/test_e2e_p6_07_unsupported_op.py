"""E2E-P6-07: 涓嶆敮鎸佹搷浣滃閿?"

鎿嶄綔鎵嬪唽:
  鐢ㄤ緥: E2E-P6-07
  鐢ㄦ埛杈撳叆: "鍏堣鍙朎:\\test_dir\\test.txt鐨勫唴瀹圭湅鐪嬮噷闈㈠啓浜嗕粈涔堬紝鐒跺悗鎶婂唴瀹瑰姞瀵嗕繚瀛樻垚鍔犲瘑鏂囦欢锛屽啀鎶婂姞瀵嗗悗鐨勬枃浠跺唴瀹硅鍑烘潵缁欐垜鐪嬬湅锛屾渶鍚庡憡璇夋垜鐢ㄤ簡浠€涔堝姞瀵嗘柟娉?"
  鍓嶇疆鏁版嵁: 鏃?Agent搴斿憡鐭ユ棤娉曟墽琛屽姞瀵?
  棰勬湡杩囩▼: Agent璇嗗埆鍑哄姞瀵嗘搷浣滀笉鏀寔 -> 鍥炲鍛婄煡鐢ㄦ埛鏃犳硶鎵ц
  閫氳繃鏍囧噯: final浜嬩欢瀛樺湪; 鍥炲璇存槑鏃犳硶鎵ц/涓嶆敮鎸? 涓嶆寰幆(steps<50)
  澶辫触鏍囧噯: Agent宕╂簝/姝诲惊鐜?鏃犳纭彁绀?
...
TEST_CASE_ID = "E2E-P6-07"
TEST_CASE_NAME = "涓嶆敮鎸佹搷浣滃閿?"
USER_INPUT = "鍏堣鍙朎:\\test_dir\\test.txt鐨勫唴瀹圭湅鐪嬮噷闈㈠啓浜嗕粈涔堬紝鐒跺悗鎶婂唴瀹瑰姞瀵嗕繚瀛樻垚鍔犲瘑鏂囦欢锛屽啀鎶婂姞瀵嗗悗鐨勬枃浠跺唴瀹硅鍑烘潵缁欐垜鐪嬬湅锛屾渶鍚庡憡璇夋垜鐢ㄤ簡浠€涔堝姞瀵嗘柟娉?"
...
@pytest.mark.asyncio
async def test_e2e_p6_07_unsupported_op():
    ...
    if not TARGET_FILE.exists():
        TARGET_FILE.write_text("杩欐槸娴嬭瘯鏂囦欢鍐呭锛岀敤浜庨獙璇佸姞瀵嗘搷浣溿€",
encoding="utf-8")

    try:
        register_pending_record(
            "E2E-P6-07", "涓嶆敮鎸佹搷浣滃閿",            USER_INPUT, {}, {}, [], [], {"errors": [], "tracebacks": []}, False,
        )
        ...
        err_keywords = ["鍔犲瘑", "涓嶆敮鎸", "鏃犳硶", "涓嶈兘", "娌 湁", "涓嶆彁渚"]
        found = [k for k in err_keywords if k in resp]
        print(f"  鍥炲鍚叧閿瘝: {found}")
        assert len(found) >= 1, f"鍥炲搴旀彁绀轰笉鏀寔鍔犲瘑鎿嶄綔(MUST), 瀹為檯鍥炲鍓?00瀛? {resp[:100]}"
        ...
        print_report(
            "E2E-P6-07", "涓嶆敮鎸佹搷浣滃閿", result, db, lc,
           ci, si, True, elapsed,
            extra={
                "Tools": tool_names,
    ...
    print(f"\n  [DONE] E2E-P6-07 {'PASSED' if passed else 'FAILED'}")
