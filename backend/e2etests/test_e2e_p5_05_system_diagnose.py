...
USER_INPUT = (
    "电脑开机越来越慢了，帮我全面诊断一下系统性能，先查一下CPU占用率、内存使用情况和磁盘空间占用情况，然后列出占用资源最多的10个进程看看谁在吃资源，再看看系统已经运行了多长时间，把以上所有的诊断结果整理成一份详细的系统诊断报告保存到E:\\test_dir。把本次任务的分析实施过程和分析结果独立生成四种版本的报告存入report目录下。"
)
