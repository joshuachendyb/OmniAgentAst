...
USER_INPUT = (
    "帮我操作一下当前的桌面环境吧。"
    "首先列出系统桌面上当前所有打开的窗口，每个窗口告诉我它的标题、所属进程和窗口句柄。"
    "然后截取当前整个屏幕的画面保存为截图文件到E:\\test_dir目录。"
    "接着读取一下系统剪贴板里当前有什么内容，不管是文本还是其他格式都告诉我。"
    "再发送一条系统通知到Windows通知栏，通知标题为'桌面自动化测试'，内容写'各项操作已完成'。"
    "最后把以上所有操作的结果汇总记录到E:\\test_dir\\desktop_op_log.txt文件中，"
    "包括窗口列表、截图文件的保存路径和大小、剪贴板内容摘要、通知发送状态。"
    "如果某个操作因为环境限制失败了也要如实记录失败原因。"
