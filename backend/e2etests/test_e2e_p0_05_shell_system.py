"""全链路E2E集成测试 - P0-05: SHELL+SYSTEM混合通路验证
...
@pytest.mark.asyncio
async def test_e2e_p0_05_shell_system():
    """P0-05: SHELL+SYSTEM混合通路"""
    ...
    try:
        ...
        assert len(result["tool_calls"]) > 0, "必须调用工具(MUST P0-05)"
        ...
        print_report(
            "E2E-P0-05", "SHELL+SYSTEM混合通路", result, db, lc,
            ci, si, True, elapsed,
        ...
        write_test_record("E2E-P0-05", "SHELL+SYSTEM混合通路(05)", user_input, r, db, ci, si, lc, passed, r.get("total_time_ms", 0)/1000.0 if r else 0, dpi=dpi, error_info=error_info)

    if passed:
        print(f"\n  [DONE] E2E-P0-05 PASSED")
