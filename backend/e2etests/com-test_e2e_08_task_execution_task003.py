"""全链路E2E集成测试 - unit-08: 任务书执行验证(task003)
严禁在脚本内设任何超时 — 统一由pytest.ini的timeout=3000管理

操作手册对照:
    用例: unit-08
     用户输入: "读取任务书E:\\test_dir\\task\\task003.txt,分析可行性和实施方案,逐一执行全部任务,做任务总结"
      前置数据: E:\test_dir\task\task003.txt存在
...
-- 小欧 2026-07-14 (基于 unit-05 复制; 与 case-05 互换读取目标: 本用例读取 task003.txt)
...
TASK_FILE = Path("E:/test_dir/task/task003.txt")  # 2026-07-26 小欧: 修复错误路径 (原task001.txt 与 user_input 的 task003.txt 不一致)


@pytest.mark.e2e_full_link
@pytest.mark.asyncio
async def test_e2e_unit_08_task_execution():
    """unit-08: 任务书执行 - 读取任务书(task003.txt)逐一执行全部任务"""
    ...
    error_info = None
    user_input = (
        '你需要读取任务书"E:\test_dir\task\task003.txt"的要求,'
        "分析任务的可行性和实施方案,"
    ...
    try:
        ...
        # unit-06核心: 必须调用readtext读取任务书
        tool_names = [t["tool_name"] for t in result["tool_calls"]]
        file_tools = {"readtext"}
        has_read = any(n in file_tools for n in tool_names)
        assert has_read, f"必须调用readtext(MUST unit-06), 实际: {tool_names}"

        # unit-06核心: 回复应包含任务总结关键词
        ...
        assert has_task_keyword, f"回复应包含任务总结关键词(MUST unit-06), 回复: {resp[:200]}"
        ...
        assert db["execution_steps_count"] >= 1, (
            f"execution_steps必须有记录(MUST unit-06), 实际={db['execution_steps_count']}"
        )
        ...
        assert len(_ti) == 0, f"DB steps中应有readtext操作(MUST unit-06): {_ti}"
