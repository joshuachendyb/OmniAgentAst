...
操作手册对照:
   用例: E2E-P0-07
    用户输入: "创建network_diag.py->执行->搜索各方向代码~30段并运行调试->读输出对比->汇总report.txt->4种文档格式(长链路)"
...
USER_INPUT = (
    ...
    "第三阶段，在网上分别搜索python在网络、文件、系统、大模型方向的各1段功能代码，"
    "下载或整理约30段代码并逐一运行调试，确保所有代码可正常运行。"
    "第四阶段，读取脚本执行输出，将结果与网上搜索到的诊断方法进行对比分析，"
    "汇总整理成一份完整的网络诊断报告保存到E:\\test_dir\\network_diagnosis_report.txt。"
    "第五阶段，读取该报告确认内容完整，并将本次任务的分析和完成过程总结"
    "以4种文档格式写到report目录下自建任务相关的目录下保存。"
)
