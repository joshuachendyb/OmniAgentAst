...
USER_INPUT = (
    "帮我把E:\\test_dir\\test.docx的内容读出来，提取出里面的所有文字内容保存到E:\\test_dir\\doc_content.txt，再把这个文档的元数据比如作者、创建日期、修改日期、页数等也提取出来保存到E:\\test_dir\\doc_metadata.txt，然后看看现在的日期和时间，计算一下从文档创建到现在过了多久，最后把完成状态发送一条通知告诉我处理完成。把本次任务的分析实施过程和分析结果独立生成四种版本的报告存入report目录下。"
)
