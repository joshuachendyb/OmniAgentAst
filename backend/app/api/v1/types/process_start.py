...
def get_status_icon(
    ...
    3. is_need_confirm_from_security=true → ⚠️ （有风险需确认，来自安全检查）
    4. 其他 → ✅ （正常，可进入thought）
...
def parse_llm_response(content: str) -> tuple[bool, bool, str]:
    ...
    Returns:
        ...
        - is_need_confirm_from_llm: 固定为 False，不使用用户确认功能
    """
    try:
        result = json.loads(content)
        is_clear = result.get('is_clear', True)
        # 🔴 is_need_confirm_from_llm 固定为 False，不使用用户确认功能
        is_need_confirm_from_llm = False
