...
2026-09-05 小健 8.5拆分后llm_stream→llm_call, 组装纯函数已移llm_response_builder.py

—— type 分类链（知识备忘 — 小欧 2026-07-15） ——
响应组装(body builder家族)在llm_response_builder.py; 本模块专注"发起LLM调用"：

  LLM 产 tool_calls → llm_response_builder._build_tool_calls_response() → {"type": "action"}
  LLM 仅文本        → llm_response_builder._build_answer_response()     → {"type": "answer"}
  流异常/出错       → llm_response_builder._yield_error_response()      → {"type": "error"}
