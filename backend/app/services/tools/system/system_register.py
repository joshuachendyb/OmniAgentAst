...
SYSTEM_TOOL_DESCRIPTIONS = {
    ...
    "net_connections": "获取网络连接列表，支持按类型（TCP/UDP）、状态（ESTABLISHED/LISTEN）、端口过滤，可获取关联进程信息，支持DNS解析控制（resolve_dns）。适合查看网络连接、排查端口占用",
    ...
    "service_list": "列出系统服务（Windows用sc/Linux用systemctl），支持按名称和状态（running/stopped）过滤，支持输出格式选择(output_format: table/json)。适合查看服务状态、管理服务",
