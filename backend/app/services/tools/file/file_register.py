...
FILE_TOOL_DESCRIPTIONS = {
    ...
    "search_files": "按文件名搜索文件，支持通配符/递归/排除模式/排序（mtime/name/size）。适合找特定文件、按时间排序找最近修改",
    "generate_report": "生成文件操作报告，记录所有操作历史",
    "copy_file": "复制文件或目录，支持递归复制、覆盖控制、保留元数据",
    "create_directory": "创建目录，自动创建父目录，已存在不报错",
    "get_file_info": "获取文件/目录的详细元数据（大小/创建时间/修改时间/类型/权限）",
    "compare_files": "比较两个文件的内容/大小/修改时间差异",
    "batch_rename": "批量重命名文件（正则匹配），支持预览和冲突处理",
    "compress_files": "压缩文件或目录为zip/tar.gz，支持加密和分卷",
    "file_monitor": "监控目录文件变化（创建/修改/删除/重命名事件）",
    "file_statistics": "统计目录的文件数量/总大小/类型分布",
    "file_checksum": "计算文件的MD5/SHA256哈希值，用于校验文件完整性",
    ...
    "list_allowed_directories": "列出系统允许访问的目录白名单",
}
