...
def parse_operation_type_v2(command: str) -> tuple:
    ...
    negation_words = {
        'DELETE': ['复制', '移动', '查看', '读取'],  # 有这些词说明不是删除
        'COPY': ['删除', '移动', '查看', '读取'],      # 有这些词说明不是复制
        'MOVE': ['复制', '删除', '查看', '读取'],      # 有这些词说明不是移动
        'EXEC': ['查看', '读取', '复制'],             # 有这些词说明不是执行
        'UPDATE': ['查看', '读取', '复制', '删除'],    # 有这些词说明不是修改
        'CREATE': ['删除', '修改', '查看', '读取'],    # 有这些词说明不是创建
    }
