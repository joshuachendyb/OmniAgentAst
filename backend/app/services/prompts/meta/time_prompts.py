...
class TimePrompts(BasePrompts):
    ...
    def get_system_prompt(self) -> str:
        ...
        logger.debug(f"[TimePrompts] 系统信息长度: {len(system_info)}")
...
1. get_time - Query and format time
   ...
   - 使用场景: "现在几点了", "今天星期几", "当前时间戳", "格式化时间", "转时间戳"
   - Examples:
     * get_time(action="now")
...
4. query_calendar - 日期综合检查
   ...
   - 使用场景: "明天是周末吗", "明天放假吗", "明天是工作日吗", "下个工作日是几号"
   - Examples:
...
6. timer - 定时器管理
   ...
   - 使用场景: "3分钟后提醒我", "设置定时器", "取消定时器", "有哪些定时器"
   - Examples:
