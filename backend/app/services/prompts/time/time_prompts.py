...
6. **【重要】任务完成后必须用finish结束**: 任务完成时，必须使用tool_name="finish"结束，如：{"thought": "任务完成", "tool_name": "finish", "tool_params": {"result": "完成摘要"}}。禁止在最终回复中包含其他工具的tool_name/tool_params，否则会被误判为又一次工具调用导致死循环
