...
from app.constants import ERR_FOCUS_WINDOW, ERR_WINDOW_NOT_FOUND


def _build_window_focus_llm_data(exec_code: str, duration_ms: int, title: str = "",
                                  err_code: str = "", detail: str = "") -> dict:
    """window_focus的llm_data构建函数 — 小健 2026-06-22"""
    if exec_code == "error":
        return {
...
def window_focus(window_title: str) -> Dict[str, Any]:
    """聚焦窗口 — 小健 2026-06-22 拆分独立文件"""
    try:
        import win32gui
    except ImportError:
        return build_error(data={"error_detail": "需要安装 pywin32 库", "params": {"window_title": window_title}}, llm_data=_build_window_focus_llm_data("error", 0, window_title, "ERR_NO_WIN32GUI"))
    ...
    try:
        ...
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_window_focus_llm_data("error", duration_ms, window_title, detail=str(e))
