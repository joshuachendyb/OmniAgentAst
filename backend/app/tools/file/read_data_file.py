...
async def read_data_file(
    ...
    """读取结构化配置文件 — 小欧 2026-06-17 — 小欧 2026-06-22 独立文件 — 小健 2026-06-22 修复计时铁规"""
    t0 = _time_mod.perf_counter()
    encoding = "utf-8"
    action = "read"
    if not file_path:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_data_file_llm_data("error", duration_ms, file_path=file_path, detail="file_path是必填参数")
    ...
    if not is_valid:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_data_file_llm_data("error", duration_ms, file_path=file_path, detail=err)
    ...
    if not detected:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_data_file_llm_data("error", duration_ms, file_path=file_path, detail=f"无法识别文件格式: {file_path},请通过format参数指定")
    ...
    if not dispatch:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_data_file_llm_data("error", duration_ms, file_path=file_path, detail=f"不支持读取{detected}格式")
    ...
    if func is None:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_data_file_llm_data("error", duration_ms, file_path=file_path, detail=f"{detected}格式不支持读取")
    ...
    try:
        ...
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_data_file_llm_data("success", duration_ms, file_path=file_path, detected_format=detected, item_count=item_count)
        ...
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_data_file_llm_data("error", duration_ms, file_path=file_path, detail=str(e))
