...
从file_tools.py拆分而来 — 小欧 2026-06-22 — 小健 2026-06-24 重命名为read_config_file
"""
...
def _build_read_config_file_llm_data(
    exec_code: str, duration_ms: int,
    file_path: str = "", detected_format: str = "",
    detail: str = "", item_count: int = 0,
) -> Dict[str, Any]:
    """read_config_file的llm_data构建函数 — 小健 2026-06-21 — 小欧 2026-06-22 — 小健 2026-06-24 重命名"""
    if exec_code == "error":
        return {
            "summary": f"读取配置文件失败: {detail}",
            "action": {"tool": "read_config_file", "tool_zh": "读配置文件", "target": file_path, "params": {}},
...
async def read_config_file(
    ...
    """读取结构化配置文件 — 小欧 2026-06-17 — 小欧 2026-06-22 独立文件 — 小健 2026-06-24 增加文件类型前置检查"""
    ...
    if not is_valid:
        ...
        llm_data = _build_read_config_file_llm_data("error", duration_ms, file_path=file_path, detail=f"{error_detail}。{hint}")
        return build_error(data={"error_detail": error_detail, "params": {"file_path": file_path}}, llm_data=llm_data)
    
    if not file_path:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_config_file_llm_data("error", duration_ms, file_path=file_path, detail="file_path是必填参数")
    ...
    if not is_valid:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_config_file_llm_data("error", duration_ms, file_path=file_path, detail=err)
    ...
    if not detected:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_config_file_llm_data("error", duration_ms, file_path=file_path, detail=f"无法识别文件格式: {file_path},请通过format参数指定")
    ...
    if not dispatch:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_config_file_llm_data("error", duration_ms, file_path=file_path, detail=f"不支持读取{detected}格式")
    ...
    if func is None:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_config_file_llm_data("error", duration_ms, file_path=file_path, detail=f"{detected}格式不支持读取")
    ...
    try:
        ...
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_config_file_llm_data("success", duration_ms, file_path=file_path, detected_format=detected, item_count=item_count)
        return build_success(
            data={"data": result_data, "format": detected, "file_path": file_path, "action": action},
            llm_data=llm_data,
        )
    except Exception as e:
        logger.error(f"[read_config_file] 执行失败: {e}")
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_read_config_file_llm_data("error", duration_ms, file_path=file_path, detail=str(e))
