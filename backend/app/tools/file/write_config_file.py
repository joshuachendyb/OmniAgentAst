...
从file_tools.py拆分而来 — 小欧 2026-06-22 — 小健 2026-06-24 重命名为write_config_file
"""
...
def _build_write_config_file_llm_data(
    exec_code: str, duration_ms: int,
    file_path: str = "", detected_format: str = "",
    detail: str = "", item_count: int = 0,
) -> Dict[str, Any]:
    """write_config_file的llm_data构建函数 — 小健 2026-06-21 — 小欧 2026-06-22 — 小健 2026-06-24 重命名"""
    if exec_code == "error":
        return {
            "summary": f"写入配置文件失败: {detail}",
            "action": {"tool": "write_config_file", "tool_zh": "写配置文件", "target": file_path, "params": {}},
...
async def write_config_file(
    ...
    if not is_valid:
        ...
        llm_data = _build_write_config_file_llm_data("error", duration_ms, file_path=file_path, detail=error_detail)
    ...
    if data is None:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_write_config_file_llm_data("error", duration_ms, file_path=file_path, detail="data是必填参数")
        return build_error(data={"error_detail": "data是必填参数", "params": {"data": data}}, llm_data=llm_data)
    is_valid, err = _validate_path(file_path)
    if not is_valid:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_write_config_file_llm_data("error", duration_ms, file_path=file_path, detail=err)
    ...
    if not detected:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_write_config_file_llm_data("error", duration_ms, file_path=file_path, detail=f"无法识别文件格式: {file_path},请通过format参数指定")
    ...
    if detected in ("ini", "xml", "properties"):
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_write_config_file_llm_data("error", duration_ms, file_path=file_path, detail=f"{detected.upper()}格式暂不支持写入")
    ...
    if not dispatch:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_write_config_file_llm_data("error", duration_ms, file_path=file_path, detail=f"不支持写入{detected}格式")
    ...
    if func is None:
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_write_config_file_llm_data("error", duration_ms, file_path=file_path, detail=f"{detected}格式不支持写入")
    ...
    try:
        ...
        try:
            ...
            bytes_written = 0
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_write_config_file_llm_data("success", duration_ms, file_path=file_path, detected_format=detected, item_count=bytes_written)
        return build_success(
            data={"data": result_data, "format": detected, "file_path": file_path, "action": action, "bytes_written": bytes_written},
            llm_data=llm_data,
        )
    except Exception as e:
        logger.error(f"[write_config_file] 执行失败: {e}")
        duration_ms = int((_time_mod.perf_counter() - t0) * 1000)
        llm_data = _build_write_config_file_llm_data("error", duration_ms, file_path=file_path, detail=str(e))
