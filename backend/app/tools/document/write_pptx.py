...
def write_pptx(
    ...
    try:
        ...
        llm_data = _build_write_pptx_llm_data("error", duration_ms, str(path), detail=str(e), hint=hint)  # 小欧 2026-07-12: 此处path经Path()重赋值为WindowsPath,须str()化后传入builder,避免action.target持有Path对象触发观察格式化len()崩溃
        return build_error(data={}, llm_data=llm_data)
