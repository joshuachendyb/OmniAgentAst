...
# =============================================================================
# 7. A2修复验证：旧格式 finish result 缺类型标准化
#    旧格式: action=finish + action_input.result
# =============================================================================

class TestOldFormatFinishResultNormalization:
    """旧格式finish时result字段类型标准化（A2修复验证 - 小健 2026-05-20）"""

    def test_old_format_finish_result_is_string(self):
        """旧格式: result是字符串 → 保持不变"""
        r = parse_react_response(json.dumps({
            "action": "finish",
            "action_input": {"result": "最终答案"}
        }))
        assert r["type"] == "answer"
        assert r["response"] == "最终答案"
        assert isinstance(r["response"], str)

    def test_old_format_finish_result_is_int(self):
        """旧格式: result是int → 转为字符串"""
        r = parse_react_response(json.dumps({
            "action": "finish",
    ...
    def test_old_format_finish_result_is_float(self):
        """旧格式: result是float → 转为字符串"""
        r = parse_react_response(json.dumps({
            "action": "finish",
            "action_input": {"result": 3.14}
        }))
        assert r["type"] == "answer"
        assert isinstance(r["response"], str)
        assert r["response"] == "3.14"

    def test_old_format_finish_result_is_bool(self):
        """旧格式: result是bool → 转为字符串"""
        r = parse_react_response(json.dumps({
            "action": "finish",
            "action_input": {"result": True}
        }))
        assert r["type"] == "answer"
        assert isinstance(r["response"], str)
        assert r["response"] == "True"

    def test_old_format_finish_result_is_dict(self):
        """旧格式: result是dict → 转为JSON字符串"""
        r = parse_react_response(json.dumps({
            "action": "finish",
            "action_input": {"result": {"status": "ok", "data": "value"}}
        }))
        assert r["type"] == "answer"
        assert isinstance(r["response"], str)
        assert '"status": "ok"' in r["response"]

    def test_old_format_finish_result_is_list(self):
        """旧格式: result是list → 转为JSON字符串"""
        r = parse_react_response(json.dumps({
            "action": "finish",
            "action_input": {"result": [1, 2, 3]}
        }))
        assert r["type"] == "answer"
        assert isinstance(r["response"], str)
        assert "[1, 2, 3]" in r["response"]
...
# =============================================================================
# 8. A3修复验证：混合文本路径 finish result 缺类型标准化
#    Handler #7: _handle_mixed_text_json 中 finish 路径
# =============================================================================

class TestMixedTextFinishResultNormalization:
    """混合文本路径finish时result字段类型标准化（A3修复验证 - 小健 2026-05-20）"""

    def test_mixed_text_finish_result_is_string(self):
        """混合文本: result是字符串 → 保持不变"""
        raw = '搜索完成\n{"tool_name": "finish", "tool_params": {"result": "北京今天晴天"}}'
        r = parse_react_response(raw)
        assert r["type"] == "answer"
        assert isinstance(r["response"], str)
    ...
    def test_mixed_text_finish_result_is_int(self):
        """混合文本: result是int → 转为字符串"""
    ...
    def test_mixed_text_finish_result_is_bool(self):
        """混合文本: result是bool → 转为字符串"""
        raw = '检查完成\n{"tool_name": "finish", "tool_params": {"result": true}}'
        r = parse_react_response(raw)
        assert r["type"] == "answer"
        assert isinstance(r["response"], str)
        assert "True" in r["response"]

    def test_mixed_text_finish_result_is_dict(self):
        """混合文本: result是dict → 转为JSON字符串"""
    ...
    def test_mixed_text_finish_result_is_list(self):
        """混合文本: result是list → 转为JSON字符串"""
        raw = '搜索完成\n{"tool_name": "finish", "tool_params": {"result": [1, 2, 3]}}'
        r = parse_react_response(raw)
        assert r["type"] == "answer"
        assert isinstance(r["response"], str)
        assert "[1, 2, 3]" in r["response"]
...
# =============================================================================
# 9. A1修复验证：旧格式使用 _process_tool_params 统一管道
# =============================================================================

class TestOldFormatToolParamsPipeline:
    """旧格式tool_params走_process_tool_params统一管道（A1修复验证 - 小健 2026-05-20）"""

    def test_old_format_action_params_normalized(self):
        """旧格式: action+action_input 参数经normalize处理"""
        r = parse_react_response(json.dumps({
            "action": "search_web",
            "action_input": {"query": "test", "num_results": 10},
            "thought": "searching"
        }))
        assert r["type"] == "action"
        assert r["tool_name"] == "search_web"
        assert isinstance(r["tool_params"], dict)
        assert r["tool_params"]["query"] == "test"

    def test_old_format_action_params_filtered(self):
        """旧格式: action_input中非参数字段(如reasoning)被过滤"""
        r = parse_react_response(json.dumps({
            "action": "search_web",
            "action_input": {"query": "test", "reasoning": "这是推理过程不应保留"},
            "thought": "searching"
        }))
        assert r["type"] == "action"
...
# =============================================================================
# 10. A4修复验证：dict输入路径使用 _process_tool_params 统一管道
# =============================================================================

class TestDictInputToolParamsPipeline:
    """dict输入路径tool_params走_process_tool_params统一管道（A4修复验证 - 小健 2026-05-20）"""

    def test_dict_input_params_normalized(self):
        """dict输入: tool_params中数值类型经normalize处理"""
        r = parse_react_response({
            "tool_name": "search_web",
            "tool_params": {"query": "test", "num_results": 10}
        })
        assert r["type"] == "action"
        assert r["tool_name"] == "search_web"
        assert isinstance(r["tool_params"], dict)
    ...
    def test_dict_input_params_filtered(self):
        """dict输入: tool_params中非参数字段(如reasoning)被过滤"""
        r = parse_react_response({
            "tool_name": "search_web",
            "tool_params": {"query": "test", "reasoning": "不应保留的推理"}
        })
        assert r["type"] == "action"
        assert "reasoning" not in r["tool_params"]

    def test_dict_input_finish_result_type_normalization(self):
        """dict输入: finish的result是dict时转为JSON字符串"""
        r = parse_react_response({
            "tool_name": "finish",
            "tool_params": {"result": {"status": "ok"}}
        })
        assert r["type"] == "answer"
        assert isinstance(r["response"], str)
...
# =============================================================================
# 11. A5修复验证：关键词路径 _parse_action 使用 _process_tool_params
# =============================================================================

class TestKeywordPathToolParamsPipeline:
    """关键词路径tool_params走_process_tool_params统一管道（A5修复验证 - 小健 2026-05-20）"""

    def test_keyword_path_params_filtered(self):
        """关键词路径: tool_params中非参数字段被过滤"""
        raw = 'Thought: 需要搜索\nAction: search_web\nAction Input: {"query": "test", "reasoning": "不应保留"}'
        r = parse_react_response(raw)
        assert r["type"] == "action"
        assert r["tool_name"] == "search_web"
        if isinstance(r["tool_params"], dict):
            assert "reasoning" not in r["tool_params"]

    def test_keyword_path_params_type_normalized(self):
        """关键词路径: tool_params中数值类型经normalize处理"""
        raw = 'Thought: 需要搜索\nAction: search_web\nAction Input: {"query": "test", "num_results": 5}'
        r = parse_react_response(raw)
        assert r["type"] == "action"
        assert r["tool_name"] == "search_web"
        assert isinstance(r["tool_params"], dict)
