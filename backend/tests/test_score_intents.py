...
class TestScoreIntents:
    ...
    def test_time_keyword_returns_meta(self):
        """时间关键词返回META（TIME已合并到META）"""
        scores = _compute_intent_scores('现在几点了')
        assert ToolCategory.META in scores
        assert scores[ToolCategory.META] > 0
    ...
    def test_chinese_keyword_higher_weight(self):
        ...
        if ToolCategory.DOCUMENT in chinese_scores and ToolCategory.DOCUMENT in english_scores:
            assert chinese_scores[ToolCategory.DOCUMENT] > 0
            assert english_scores[ToolCategory.DOCUMENT] > 0
