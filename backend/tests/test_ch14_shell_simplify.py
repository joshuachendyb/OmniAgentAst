...
from app.services.tools.shell.shell_register import TOOL_INPUT_MODELS as _  # noqa
...
from app.services.tools.shell import shell_schema
...
class TestShellRegisterAllExport:
    ...
    def test_register_all_exact_contents(self):
        ...
        expected = ["_register_shell_tools", "execute_shell_command", "find_command", "shell_session", "execute_code"]
    ...
    def test_register_only_4_llm_tools(self):
        """shell_register只注册4个LLM工具描述 - 小健 2026-05-22 (execute_python+execute_javascript合并为execute_code)"""
        expected_keys = {"execute_shell_command", "find_command", "shell_session", "execute_code"}
...
class TestDemotedTools:
    ...
    def test_demoted_schemas_not_in_register(self):
        """降级Schema不应在注册表中引用 - 小健 2026-05-18"""
        source = shell_register.__file__
        import pathlib
        content = pathlib.Path(source).read_text(encoding="utf-8")
        assert "GetWorkingDirectoryInput" not in content or "降级" in content
...
class TestShellHelperMigration:
    ...
    def test_shell_helper_injection_patterns(self):
        """shell_helper的注入模式应不为空"""
        assert len(helper_patterns) > 0
...
class TestShellInitPyAllStrict:
    ...
    def test_init_all_exact_contents(self):
        ...
        expected = ["execute_shell_command", "find_command", "shell_session", "execute_code"]
...
class TestShellSchemaConsistency:
    ...
    def test_deprecated_schemas_still_importable(self):
        """验证废弃Schema已完全删除（符合禁止向后兼容原则）— 小沈 2026-05-19"""
        assert not hasattr(shell_schema, "CheckCommandAvailableInput"), "CheckCommandAvailableInput应已删除"
        assert not hasattr(shell_schema, "LocateCommandInput"), "LocateCommandInput应已删除"
        assert not hasattr(shell_schema, "GetShellOutputInput"), "GetShellOutputInput应已删除"
        assert not hasattr(shell_schema, "TerminateShellInput"), "TerminateShellInput应已删除"
