"""
execute_tool_with_retry 跨分类 fallback 测试

TDD: RED阶段 - 测试因缺少fallback逻辑而失败
Author: 小沈 - 2026-04-30
Updated: 小沈 + 小健 - 2026-06-08 (迁移至ToolRetryEngine)
"""
import pytest
...
class TestToolRetryEngineFallback:
    """测试ToolRetryEngine跨分类fallback"""
    ...
    async def test_local_tool_found_executes_ok(self):
        ...
        result = await engine.execute_tool_with_retry("my_tool", {})
        # 如果参数校验通过，能正常执行
        assert result is not None
        # 可能因参数校验返回错误信息，但不是"tool not found"
        if "ERR_" in result.get("code", ""):
            assert "Unknown tool" not in result.get("message", "")
    ...
    async def test_fallback_tool_cached_locally(self):
        ...
        with patch('app.services.tools.registry.tool_registry.get_implementation') as mock_get:
            ...
            result1 = await engine.execute_tool_with_retry("execute_command", {"command": "echo hello"})
    ...
    async def test_local_tool_not_found_fallback_to_registry(self):
        ...
        with patch('app.services.tools.registry.tool_registry.get_implementation') as mock_get:
            ...
            result = await engine.execute_tool_with_retry("execute_command", {"command": "echo hello"})
    ...
    async def test_nonexistent_tool_returns_error(self):
        ...
        tools = {}
        engine = ToolRetryEngine(tools)
        result = await engine.execute_tool_with_retry("nonexistent_tool_xyz", {})
        assert result["code"] == "ERR_TOOL_NOT_FOUND"
    ...
    async def test_tool_name_finish_bypasses_tool_lookup(self):
        ...
        tools = {}
        engine = ToolRetryEngine(tools)
        result = await engine.execute_tool_with_retry("finish", {"result": "done"})
