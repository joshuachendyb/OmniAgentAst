...
async def test_fc_retry_success():
    ...
    async def mock_request_stream(**kwargs):
        ...
        call_count += 1
        if call_count == 1:
            raise FCFormatError(message="tool_calls参数解析失败", details={"failed_tools": ["read_text_file"]})
...
def test_fc_format_error_exception():
    ...
    error = FCFormatError(message="test error")
    assert str(error) == "test error"
    assert isinstance(error, Exception)
    assert error.details == {}

    error_with_details = FCFormatError(message="test error", details={"failed_parses": ["tool1"]})
    ...
    with pytest.raises(FCFormatError, match="解析失败"):
        raise FCFormatError(message="tool_calls解析失败")
