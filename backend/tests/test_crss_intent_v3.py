...
class TestCRSSV3TypeActionDisambiguation:
    ...
    def test_read_file(self):
        """读取文件→FILE（不是DOCUMENT）"""
        scores = _compute_intent_scores("读取文件")
        assert scores.get(ToolCategory.FILE, 0) > scores.get(ToolCategory.DOCUMENT, 0)

    def test_read_database(self):
        """读取数据库→DOCUMENT（不是FILE）"""
        scores = _compute_intent_scores("读取数据库")
        assert scores.get(ToolCategory.DOCUMENT, 0) > scores.get(ToolCategory.FILE, 0)
    ...
    def test_query_database(self):
        ...
        assert primary == ToolCategory.DOCUMENT

    def test_run_shell(self):
        """运行脚本→SYSTEM"""
        primary, _, _ = detect_intent_v2("运行python脚本")
        assert primary == ToolCategory.SYSTEM
...
class TestCRSSV3MultiCategory:
    ...
    def test_file_and_database_both_scored(self):
        ...
        assert ToolCategory.DOCUMENT in scores
        assert len(scores) >= 2
