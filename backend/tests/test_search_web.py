"""
search_web 多引擎fallback测试 - 2026-05-27 小欧 重构适配Parallel/Exa/Bing架构
Author: 小沈 - 2026-05-09
Update: 小欧 - 2026-05-27
"""
import pytest
...
class TestSearchWebFallback:
    @pytest.mark.asyncio
    async def test_parallel_success_returns_parallel(self):
        from app.services.tools.network import network_tools
        mock_results = [{"title": "Test Result", "url": "http://example.com", "snippet": "Test"}]
        with patch("app.services.tools.network.network_tools._search_mcp_engine", new_callable=AsyncMock, return_value=mock_results):
            result = await network_tools.search_web(query="test query")
        assert result["code"] == "SUCCESS"
        assert result["data"]["engine"] == "Parallel"

    @pytest.mark.asyncio
    async def test_parallel_fails_falls_back_to_exa(self):
        from app.services.tools.network import network_tools
        mock_results = [{"title": "Exa Result", "url": "http://exa.com", "snippet": "Exa Test"}]
        with patch("app.services.tools.network.network_tools._search_mcp_engine", new_callable=AsyncMock) as mock_mcp:
            mock_mcp.side_effect = [None, mock_results]
            result = await network_tools.search_web(query="test query")
        assert result["code"] == "SUCCESS"
        assert result["data"]["engine"] == "Exa"

    @pytest.mark.asyncio
    async def test_all_engines_empty(self):
        from app.services.tools.network import network_tools
        with patch("app.services.tools.network.network_tools._search_mcp_engine", new_callable=AsyncMock, return_value=None):
