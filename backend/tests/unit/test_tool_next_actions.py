...
- 小健 2026-05-17
- 小健 2026-05-22: list_windows → window_info, execute_python/execute_javascript → execute_code
...
覆盖:
  ...
  desktop工具: window_info, window_control, mouse_control, screen_capture, clipboard_control
  code_execution工具: execute_code
"""
...
from app.services.tools.desktop.desktop_tools import window_info, find_windows_by_title, check_win32_platform
...
class TestDesktopNextActions:
    ...
    def test_window_info_next_actions(self):
        with patch("app.services.tools.desktop.desktop_tools.check_win32_platform", return_value=None):
            with patch("app.services.tools.desktop.desktop_tools._win32gui.EnumWindows",
                       return_value=None):
                result = window_info(action="list")
                assert_next_actions_format(result)
                if result["code"] == "SUCCESS":
                    tools = [a["tool"] for a in result["next_actions"]]
                    assert "window_info" in tools

    def test_mouse_control_next_actions(self):
        with patch("app.services.tools.desktop.gui_tools._click",
    ...
    def test_screen_capture_next_actions(self):
        with patch("app.services.tools.desktop.gui_tools._screenshot",
                   return_value={"code": "SUCCESS", "data": {}, "message": "ok"}):
            result = screen_capture()
            assert_next_actions_format(result)

    def test_clipboard_control_next_actions(self):
        with patch("app.services.tools.desktop.gui_tools._read_clipboard",
                   return_value={"code": "SUCCESS", "data": {"content": "data"}, "message": "ok"}):
...
class TestCodeExecNextActions:
    ...
    def test_execute_code_python_success_next_actions(self):
    ...
    def test_execute_code_fail_next_actions(self):
        result = execute_code(code="print(;", language="python")
        assert_next_actions_format(result, expect_present=False)

    def test_execute_code_javascript_success_next_actions(self):
