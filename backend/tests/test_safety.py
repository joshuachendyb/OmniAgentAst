...
class TestFileSafetyConfig:
    ...
    def test_default_paths(self):
        ...
        assert "omniagent" in str(db._db_paths["operations"]).lower()
...
class TestOperationRecording:
    ...
    def test_record_operation(self, safety_service):
        ...
        assert operation.task_id == "test-session"
    ...
    def test_get_session_operations(self, safety_service):
        ...
        # 使用唯一的task_id隔离测试数据
        import uuid
        task_id = f"test-session-{uuid.uuid4().hex[:8]}"
        for i in range(3):
        ...
        operations = safety_service.get_session_operations(task_id)
    ...
    def test_operation_sequencing(self, safety_service):
        """测试操作序号"""
        import uuid
        task_id = f"seq-test-{uuid.uuid4().hex[:8]}"
...
class TestBackupAndRollback:
    ...
    def safety_service_with_temp(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with patch.object(db, '_db_paths', {**db._db_paths, "operations": tmp_path / "test.db"}):
    ...
    def test_rollback_session(self, safety_service_with_temp):
        ...
        safety, tmp_path = safety_service_with_temp
        import uuid
        task_id = f"rollback-test-{uuid.uuid4().hex[:8]}"
        ...
        # 回滚整个会话
        result = safety.rollback_session(task_id)
...
class TestSpaceImpactCalculation:
    ...
    def test_create_operation_space_impact(self, safety_service):
        """测试创建操作的空间影响"""
        import uuid
        task_id = f"space-test-{uuid.uuid4().hex[:8]}"
        with tempfile.TemporaryDirectory() as tmp_dir:
    ...
    def test_delete_operation_space_impact(self, safety_service):
        """测试删除操作的空间影响"""
        import uuid
        task_id = f"space-delete-{uuid.uuid4().hex[:8]}"
        with tempfile.TemporaryDirectory() as tmp_dir:
