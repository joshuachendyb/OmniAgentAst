# -*- coding: utf-8 -*-
"""
API层业务逻辑深度测试 — chat_router / chat_stream_v2 / execute_tool

覆盖目标:
- chat_router.py (69% → 100%)
- chat_stream_v2.py (24% → 100%)
...
from app.api.v1.chat.chat_stream_v2 import (
    chat_stream_v2,
)
...
# ====================================================================
# 第二部分: chat_stream_v2 流式测试
# ====================================================================

class TestChatStreamV2:
    """chat_stream_v2 核心业务流测试"""
    ...
    async def test_empty_messages_returns_error(self, mock_valid_request):
        ...
        response = await chat_stream_v2(mock_valid_request)
    ...
    async def test_single_message_generates_stream(self, mock_valid_request, mock_ai_service):
        ...
        with (
            patch("app.api.v1.chat.chat_stream_v2.get_service", return_value=mock_ai_service),
            patch("app.api.v1.chat.chat_stream_v2.register_task", new_callable=AsyncMock),
            patch("app.api.v1.chat.chat_stream_v2.task_interrupt_check", new_callable=AsyncMock, return_value=(False, "")),
            patch("app.api.v1.chat.chat_stream_v2.task_pause_check_and_yield", new_callable=AsyncMock, return_value=mock_sse_gen()),
            patch("app.api.v1.chat.chat_stream_v2.step_start", new_callable=AsyncMock, return_value=mock_start_gen()),
            patch("app.api.v1.chat.chat_stream_v2.run_sse_stream", new_callable=AsyncMock, return_value=mock_sse_gen()),
            patch("app.api.v1.chat.chat_stream_v2.task_cancel_check_and_yield", new_callable=AsyncMock, return_value=None),
            patch("app.api.v1.chat.chat_stream_v2.task_cleanup", new_callable=AsyncMock),
        ):
            response = await chat_stream_v2(mock_valid_request)
            assert response.status_code == 200
    ...
    async def test_stream_with_session_id_preserves_session(self, mock_ai_service):
        ...
        with (
            patch("app.api.v1.chat.chat_stream_v2.get_service", return_value=mock_ai_service),
            patch("app.api.v1.chat.chat_stream_v2.register_task", new_callable=AsyncMock) as mock_reg,
            patch("app.api.v1.chat.chat_stream_v2.task_interrupt_check", new_callable=AsyncMock, return_value=(False, "")),
            patch("app.api.v1.chat.chat_stream_v2.task_pause_check_and_yield", return_value=empty_ag()),
            patch("app.api.v1.chat.chat_stream_v2.step_start", return_value=empty_ag()),
            patch("app.api.v1.chat.chat_stream_v2.run_sse_stream", return_value=empty_ag()),
            patch("app.api.v1.chat.chat_stream_v2.task_cancel_check_and_yield", new_callable=AsyncMock, return_value=None),
            patch("app.api.v1.chat.chat_stream_v2.task_cleanup", new_callable=AsyncMock) as mock_clean,
        ):
            response = await chat_stream_v2(request)
            assert response.status_code == 200
    ...
    async def test_stream_exception_captured_and_cleaned_up(self, mock_valid_request, mock_ai_service):
        ...
        with (
            patch("app.api.v1.chat.chat_stream_v2.get_service", return_value=mock_ai_service),
            patch("app.api.v1.chat.chat_stream_v2.register_task", new_callable=AsyncMock, side_effect=RuntimeError("注册失败")),
            patch("app.api.v1.chat.chat_stream_v2.task_cleanup", new_callable=AsyncMock) as mock_clean,
        ):
            response = await chat_stream_v2(mock_valid_request)
...
class TestBusinessFlowIntegration:
    ...
    async def test_full_stream_flow_with_mocked_steps(self, mock_valid_request, mock_ai_service):
        ...
        with (
            ...
            patch("app.api.v1.chat.chat_stream_v2.task_interrupt_check", new_callable=AsyncMock, return_value=(False, "")),
            patch("app.api.v1.chat.chat_stream_v2.task_pause_check_and_yield", return_value=empty_ag()),
            patch("app.api.v1.chat.chat_stream_v2.step_start", return_value=mock_start_gen()),
            patch("app.api.v1.chat.chat_stream_v2.run_sse_stream", return_value=mock_sse_stream_gen()),
            patch("app.api.v1.chat.chat_stream_v2.task_cancel_check_and_yield", new_callable=AsyncMock, return_value=None),
            patch("app.api.v1.chat.chat_stream_v2.task_cleanup", new_callable=AsyncMock),
        ):
            response = await chat_stream_v2(mock_valid_request)
    ...
    async def test_full_stream_flow_with_error_event(self, mock_valid_request, mock_ai_service):
        ...
        with (
            ...
            patch("app.api.v1.chat.chat_stream_v2.task_pause_check_and_yield", return_value=empty_ag()),
            patch("app.api.v1.chat.chat_stream_v2.step_start", return_value=empty_ag()),
            patch("app.api.v1.chat.chat_stream_v2.run_sse_stream", return_value=mock_error_sse_gen()),
            patch("app.api.v1.chat.chat_stream_v2.task_cancel_check_and_yield", new_callable=AsyncMock, return_value=None),
            patch("app.api.v1.chat.chat_stream_v2.task_cleanup", new_callable=AsyncMock),
        ):
            response = await chat_stream_v2(mock_valid_request)
    ...
    async def test_multiple_messages_uses_last_as_user_input(self, mock_ai_service):
        ...
        with (
            ...
            patch("app.api.v1.chat.chat_stream_v2.task_interrupt_check", new_callable=AsyncMock, return_value=(False, "")),
            patch("app.api.v1.chat.chat_stream_v2.task_pause_check_and_yield", return_value=empty_ag()),
            patch("app.api.v1.chat.chat_stream_v2.step_start", side_effect=capture_step_start),
            patch("app.api.v1.chat.chat_stream_v2.run_sse_stream", return_value=empty_ag()),
            patch("app.api.v1.chat.chat_stream_v2.task_cleanup", new_callable=AsyncMock),
        ):
            response = await chat_stream_v2(req)
            async for _ in response.body_iterator:
                pass
    ...
    async def test_interrupt_flow_path(self, mock_valid_request, mock_ai_service):
        ...
        with (
            patch("app.api.v1.chat.chat_stream_v2.get_service", return_value=mock_ai_service),
            patch("app.api.v1.chat.chat_stream_v2.register_task", new_callable=AsyncMock),
            patch("app.api.v1.chat.chat_stream_v2.task_interrupt_check", new_callable=AsyncMock, return_value=(True, interrupt_msg)),
            patch("app.api.v1.chat.chat_stream_v2.task_pause_check_and_yield", return_value=empty_ag()),
            patch("app.api.v1.chat.chat_stream_v2.step_start", return_value=empty_ag()),
            patch("app.api.v1.chat.chat_stream_v2.run_sse_stream", return_value=empty_ag()),
            patch("app.api.v1.chat.chat_stream_v2.task_cleanup", new_callable=AsyncMock) as mock_clean,
