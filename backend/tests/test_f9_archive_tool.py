...
async def test_archive_compress():
    ...
    result = await ft.archive_tool(action="compress", source=src_dir, destination=zip_path)
    data = _ok(result)
    assert result["code"] == "SUCCESS", f"压缩失败: {result.get('message', '')}"
...
async def test_archive_extract():
    ...
    zip_path = os.path.join(tempfile.mkdtemp(), "test.zip")
    await ft.archive_tool(action="compress", source=src_dir, destination=zip_path)

    out_dir = tempfile.mkdtemp()
    result = await ft.archive_tool(action="extract", source=zip_path, destination=out_dir)
    assert result["code"] == "SUCCESS", f"解压失败: {result.get('message', '')}"
    data = _ok(result)
...
async def test_archive_invalid_action():
    ...
    assert "不支持的action" in result.get("message", "")
...
async def test_archive_compress_no_source_path():
    ...
    data = _ok(result)
    assert result["code"] != "SUCCESS"
    assert "source" in result.get("message", "")
