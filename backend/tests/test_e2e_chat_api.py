...
class TestEndToEndChatAPI:
    ...
    @pytest.mark.skip(reason="Windows async超时卡死，response.text流式读取不稳定")
    @pytest.mark.asyncio
    ...
    @pytest.mark.skip(reason="Windows async超时卡死，aiter_lines流式读取不稳定")
    @pytest.mark.asyncio
    async def test_chat_sse_contains_start_step(self, mock_llm):
        """验证SSE流第一个事件是start类型"""
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            async with client.stream("POST", "/api/v1/chat/stream/v2",
                json={"messages": [{"role": "user", "content": "测试start步骤"}], "stream": True},
                timeout=15
            ) as response:
                    ...
                    if line.startswith("data: ") and line[6:].strip():
                        try:
                            ...
                            if data.get("type") == "start":
                                print(f"  首个事件: type={data['type']}, step={data.get('step')}")
                                return
                        except:
                            pass
        pytest.fail("未找到start事件")
