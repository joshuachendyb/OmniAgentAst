...
async def test_copy():
    ...
    data = _ok(result)
    assert result["code"] == "SUCCESS", f"复制失败: {result.get('message', '')}"
...
async def test_move():
    ...
    data = _ok(result)
    assert result["code"] == "SUCCESS", f"移动失败: {result.get('message', '')}"
...
async def test_delete():
    ...
    data = _ok(result)
    assert result["code"] == "SUCCESS", f"删除失败: {result.get('message', '')}"
