...
| 版本 | 时间 | 更新内容 | 作者 |
|------|------|---------|------|
| v1.2 | 2026-07-14 21:10:00 | 第十章根因更正:Errno22真实根因为time_utils.ensure_timestamp_milliseconds潜伏bug(Python3.13宽松fromisoformat+漏捕OSError),非WAL-shm;方法二/VACUUM为误诊白做 | 小欧 |
...
**更正时间**: 2026-07-14 21:10:00
**更正人**: 小欧

### 10.1 原诊断错误

本文第一、三章将 `case05/Errno22` 根因归于"WAL 模式 `-shm` 共享内存在 Windows 2GB+ 库并发读写下 `GetFileInformationByHandleEx` 不稳定"。经实测 traceback 复核，**该诊断错误**。

### 10.2 真实根因（铁证）

`GET /sessions/{id}/messages` 失败时后端日志与实测 traceback 均指向：

```
File "app\utils\time_utils.py", line 49, in ensure_timestamp_milliseconds
    return int(datetime.fromisoformat(str(ts_value).replace(' ', 'T')).timestamp() * 1000)
OSError: [Errno 22] Invalid argument
```

`[Errno 22]` 来自 `datetime.timestamp()`（底层 `time.mktime`），**非 SQLite 文件 I/O**。机制链：

1. `chat_messages.timestamp` 列类型为 **TEXT**，`create_timestamp()` 返回 epoch 毫秒**整数**插入后被 SQLite 转存为 TEXT 字符串（如 `'1784031800406'`）。该存储格式 10 天 100 次测试未变，非本次改动引入。
2. `get_session_messages` 读路径调用 `ensure_timestamp_milliseconds(row['timestamp'])`，传入 TEXT 串。
3. 该函数 `except (ValueError, TypeError, OverflowError)` **漏捕获 `OSError`**。
4. Python 3.13 的 `fromisoformat` 变宽松，将 13 位 epoch 串误解析为约公元 1784 年的 datetime；`.timestamp()` 因年份早于 Windows `mktime` 下限(1970) 抛 **OSError 22**，未被 except 捕获 → 500 错误。
5. **偶发性**：取决于具体数字。多数 13 位串被解析成"非法日期"→ `ValueError`（被捕获→静默返回当前时间，不崩）；少数串（如 case04 的 `'1784031800406'`）被解析成"合法但 <1970 的日期"→ `OSError`（未捕获→崩）。

### 10.3 真正修复

位置：`backend/app/utils/time_utils.py` `ensure_timestamp_milliseconds`（自 2026-06-07 小沈写入后未变，为潜伏 bug）。

修复内容：
- 数字串（epoch 毫秒/秒）直接 `int()` 转换，不再走 `fromisoformat`；
- `except` 补充捕获 `OSError`。

验证：case04/05/06 在修复后全部 PASS（EXIT_CODE=0），`Errno 22` 日志归零。

### 10.4 方法二/VACUUM 结论

`database.py` 的 `journal_mode=DELETE` 改动与 `VACUUM` 运维步骤**针对误诊根因**，属白做但无害：
- DELETE 模式本身未消除该 `Errno 22`（VACUUM 后重跑 case04 仍报 `Errno 22`，直至 `time_utils` 修复才消失）；
- VACUUM 仅起碎片整理/回收空间作用（2.4GB→约 2GB），保留 DELETE 模式无碍，故不予回退。

### 10.5 与步骤表改造(方法一A)的关系

方法一A（独立步骤表）是**正确的建模改进**（根治 R3 大写入/R4 读放大），与 `Errno 22` 无因果关系。其实施不受本次根因更正影响，保持有效。
