...
4. Phase 0需新增ToolMetadata.safety_level字段并补标58个工具的安全等级（小沈审查v1.2：2天紧张但可行，建议优先标注DANGEROUS/DESTRUCTIVE，§6.3预估约15个，以Phase 0实际标注为准）
...
| 改造项 | 说明 | 工作量 | 来源 |
|--------|------|--------|------|
| SSE事件扩展 | 在useSSE(1648行)中新增`authorization_required`事件类型处理，需侵入核心事件循环(以useSSE实际事件数为准) | 1天 | Agent融合方案 + 小沈审查v1.2 |
