...
下游规则系统（command_security黑名单172条 + CRSS 175关键词）是精确匹配，错别字=安全漏洞/路由失败。必须用L1规则矫正(<1ms)在LLM处理前修正输入，安全关键词优先。当前PreprocessingPipeline仅做strip()，等于裸奔。
...
Phase 0(安全分级标注2.5天)→Phase 3(TextCorrectorV2 1天)→Phase 4(SemanticRouter 1.5天)→Phase 5(ToolSafetyLayer+HITL 2天)→Phase 6(ChatRouter改造+灰度 2天)→Phase 7(前端HITL 3天)→Phase 8(重复执行消除 2天)→Phase 10(测试 2天)≈16天。Phase 1/2/9已实施移除。

**1.1.8 前端改造 → 详见§十**

SSE事件扩展(1648行useSSE新增authorization_required)+安全确认弹窗+授权API+SessionTrust复选框+异常暂停提示≈4天。
...
LLM自己能理解错别字，但command_security黑名单(172条)和CRSS(175关键词)都是**精确匹配**，矫正不是帮LLM理解，是**让规则系统生效**。
...
DANGEROUS_VOCAB_DEFAULT = ["删除", "格式化", "关闭", "重启", "清除", "卸载", "rm", "format"]
...
| 场景 | 检测结果 | 准确率 |
|------|---------|--------|
| "帮我删处文件" | pinyin: "删处"→"删除" ✅ | 99%+（同音字是中文笔误主因） |
| "帮我姗除文件" | pinyin: "姗除"→"删除" ✅ | 99%+（拼音相同自动识别） |
| "帮我册除文件" | visual: "册除"→"删除" ✅ | 95%（需维护视觉相似表） |
| "帮我删除文件" | 无模糊匹配，原样通过 ✅ | 100% |
| "帮我册子文件" | "册子"编辑距离1但非危险词 ✅ | 不误触（只匹配DANGEROUS_VOCAB） |

**结论**：拼音+编辑距离+视觉相似 三重检测，**不依赖枚举错别字**，对中文笔误（同音字为主）召回率可达99%+。视觉相似表只需维护约20组常见形近字，是有限可维护的工作量。
...
- 已有**4级风险分级**(safe/medium/high/critical) + CRSS权重评分(0-10分) + 危险命令黑名单(172条) + 系统关键目录检查
...
6. 全部运行2周无问题 → 删除旧代码(CRSS/旧PreprocessingPipeline/IntentClassifier类)
...
| 改造项 | 说明 | 工作量 | 来源 |
|--------|------|--------|------|
| SSE事件扩展 | 在useSSE(1648行)中新增`authorization_required`事件类型处理，需侵入核心事件循环(switch-case 12种事件) | 1天 | Agent融合方案 + 小沈审查v1.2 |
...
| 实施工期 | ~16天 | | | 小沈审查v1.6修正(Phase1/2/9已完成移除) |
...
| 上下文窗口浪费 | 477KB/9轮 | **~50KB/9轮** | **-90%(预估)** | 重复执行分析 |
