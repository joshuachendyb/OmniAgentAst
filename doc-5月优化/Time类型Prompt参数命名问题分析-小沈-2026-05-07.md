...
| # | 改造目标 | 涉及通道 | 涉及源文件 | 当前问题 | 改造方法 | 范围 |
|---|---------|---------|-----------|---------|---------|------|
| G5 | **去掉 get_system_prompt() 的重复参数** | 通道 1 | ③ 各 `*_prompts.py:get_system_prompt()` | 工具描述中手写 `- Parameters:` 参数行，跟通道 3 的 Pydantic 重复 | 去掉 `- Parameters:` 的参数行，只保留用途说明和示例 | **10个 prompt 文件** |
| G6 | **get_parameter_reminder() 改为自动生成** | 通道 1 | ④ 各 `*_prompts.py:get_parameter_reminder()` + `registry.py` | 手写参数列表，跟通道 3 的 Pydantic 不同步 | 改为调用 `tool_registry.generate_param_reminder(category)` 从 Pydantic 自动生成。**⚠ 前置条件：G1 必须先修复** | **10个 prompt 文件** |
...
| **参数信息完整** | ✅ 类型 + required/optional + 语义默认值都有了 | 语义默认值从 description 正则提取 |
...
**涉及文件**：
- `registry.py` — **新增** `generate_param_reminder()` + `_extract_semantic_default()`
- 10个 prompt 文件的 `get_parameter_reminder()` 改为调用自动生成（同 G5 范围）

**Step A**：在 `registry.py` 中新增：

```python
def _extract_semantic_default(description: str) -> str:
    """从 Field description 中提取语义默认值"""
    if not description:
        return ""
    import re
    m = re.search(r'默认[为]?([^，。；,.;\n]{1,20})', description)
    if m:
        val = m.group(1).strip()
        if val and val not in ("None", "null", "无"):
            return "default=" + val
    return ""
...
def generate_param_reminder(self, category=None) -> str:
    ...
    for name, meta in sorted(self._tools.items(), key=lambda x: x[0]):
        ...
        for pname, pinfo in schema["properties"].items():
            ...
            semantic_default = _extract_semantic_default(pinfo.get("description", ""))
            if semantic_default:
                param_parts.append("{}({}, {}, {})".format(pname, req_str, ptype, semantic_default))
            else:
