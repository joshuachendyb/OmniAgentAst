...
| 步骤 | 内容 | 涉及章节 | 依赖 | 效果 |
|------|------|---------|------|------|
| **第一步** | G1+G2+G3（修复通道3的schema质量：Union类型、description截断、无差别过滤） | 第13章 | 无 | 通道3的数据质量修复，为后续FC启用做准备 |
| **第二步** | G4+G5+G7（清理冗余：register参数说明、prompt的-Parameters:块、summary参数名） | 第13章 | 无，与第一步并行 | 减少token消耗，消除重复信息 |
| **第三步** | FC通道启用（to_openai_tools + adapter注入 + 删除子类重复代码） | **第12章** | **G4必须先完成**（G4删register参数说明→改变meta.description→影响to_openai_tools()的输出）；通道3已修复（第一步） | LLM正式收到tools JSON Schema，通过required明确参数必填性 |
| **第四步** | G6（get_parameter_reminder()改为自动生成） | 第13章 | **G1必须先完成** | Param Reminder与Pydantic同步，消除手写差异 |

#### 关键说明

1. **FC启用（第三步）不强制等待G1~G3完成**。即使不修G1，FC也能工作（只是schema中的Union类型会被简化）。
2. **但G4必须在FC启用之前**。G4删除register的"参数说明"段落，会改变`meta.description`。而`to_openai_tools()`直接用`meta.description`作为`function.description`。如果FC先启用再删G4，LLM会先收到冗余参数说明的description。
3. **G1必须在G6之前**。G6依赖Pydantic schema生成参数文本，如果Union类型被简化，自动生成的结果比手写版还差。
4. **G6在FC启用后的角色变化**：FC启用后，通道3已经向LLM提供了完整的参数信息（结构化），通道1⑥的Parameter Reminder文本退化为辅助角色。G6改为生成简洁版reminder即可，不需要像手写版那样详细。
5. **G4+G5+G7（第二步）在任何情况下都有收益**——减少token消耗、消除重复信息，与是否启用FC无关。
