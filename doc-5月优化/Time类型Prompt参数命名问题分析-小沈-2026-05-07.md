...
| **第三步** | FC通道启用（to_openai_tools + adapter注入 + 删除子类重复代码） | **第12章** | 通道3已修复（第一步） | LLM正式收到tools JSON Schema，通过required明确参数必填性 |
| **第四步** | G6（get_parameter_reminder()改为自动生成） | 第13章 | **G1必须先完成** | Param Reminder与Pydantic同步，消除手写差异 |

#### 关键说明

1. **FC启用（第三步）不强制等待G1~G7完成**。即使不修G1，FC也能工作（只是schema中的Union类型会被简化）。
2. **但G1必须在G6之前**。G6依赖Pydantic schema生成参数文本，如果Union类型被简化，自动生成的结果比手写版还差。
3. **G6在FC启用后的角色变化**：FC启用后，通道3已经向LLM提供了完整的参数信息（结构化），通道1⑥的Parameter Reminder文本退化为辅助角色。G6改为生成简洁版reminder即可，不需要像手写版那样详细。
4. **G4+G5+G7（第二步）在任何情况下都有收益**——减少token消耗、消除重复信息，与是否启用FC无关。
...
| **通道 3** | API `tools` 参数（JSON Schema 结构化） | `react_schema.py:get_tools_schema_for_function_calling()` 组装 `openai_tools` 数组 → 传给 LLM 的 `request_json["tools"]` | `function.name`（工具名）+ `function.description`（来自 register）+ `function.parameters.properties`（来自 Pydantic）+ `function.parameters.required`（来自 Pydantic） | ① `time_schema.py`（Pydantic 模型 → `input_schema`）+ ② `time_register.py`（`TIME_TOOL_DESCRIPTIONS` → `description`）→ `registry.py` → `react_schema.py`（组装） | ✅ `properties/required` 由 Pydantic 自动生成；但②的 `description` 含重复参数说明 |
