...
```
conversation_history = [
    {"role": "system", "content": sys_prompt},   # 第1条：系统提示（7182字符）
    {"role": "user", "content": task_prompt}     # 第2条：任务提示（274字符）
]
```

### 2.3 sys_prompt 内部层次（7182字符）

**sys_prompt由6个段落组成**：

| 序号 | 段落名称 | 位置 | 内容示例 | 字符数 |
|------|---------|------|---------|--------|
| ① | 系统信息 | 开头 | `【当前系统】Windows...` | ~300 |
| ② | Tool详细描述 | L19-130 | 每个工具的参数/示例/场景 | ~3500 |
| ③ | Tool Call Examples | L97-105 | 3个完整JSON示例 | ~500 |
| ④ | Safety提醒 | L131 | `⚠️ Time Safety...` | ~100 |
| ⑤ | **Parameter Reminder** | **L133-143** | **参数名列表** | **~200** |
| ⑥ | TERMINATION RULE | L146+ | finish退出规则 | ~300 |

### 2.4 task_prompt 组成（274字符）
...
├── message[0] {"role": "system"}  → sys_prompt (第1条，内部6个段落)
│   ├── ① 系统信息
│   ├── ② Tool详细描述
│   ├── ③ Tool Call Examples
│   ├── ④ Safety提醒
...
═══════════════════════════════════════════════════════════
conversation_history = [
    [0] {"role": "system"} → sys_prompt (7182字符)
        ├── ① 系统信息
        ├── ② Tool详细描述
        ├── ③ Tool Call Examples
        ├── ④ Safety提醒
...
段落2 (6879字符):
  ...
  ...
  L133: Parameter Reminder:
  L136: - time_format: timestamp, pattern          ← 问题所在！只有参数名
  L142: - time_is_weekend: date                    ← 问题所在！只有参数名
```
...
### 4.1 Parameter Reminder（当前）

```python
# time_prompts.py L144-157
...
| 层次 | 信息量 | 位置 | LLM可见性 |
|------|--------|------|----------|
| Parameter Reminder | ★☆☆ (只有参数名) | prompt尾部 | **高**（调用时记忆清晰） |
| Prompt详细描述 | ★★★ (参数+类型+示例) | prompt中部 | 中（被大量文字淹没） |
| Schema定义 | ★★☆ (参数+类型) | tool_registry | 低（不直接给LLM） |

**矛盾**：信息量最少的Parameter Reminder，LLM反而最容易记住（位置靠后+调用时最近记忆）

---

## 五、改进方案

### 5.1 方案A：增强Parameter Reminder（推荐）

**修改位置**：`time_prompts.py` L149/L155
...
```
【强制】必须使用schema中定义的参数名，禁止自创参数如date/path
```

**优点**：直接明确约束  
**缺点**：增加prompt长度，可能被LLM忽略
...
1. **治本**：增强Parameter Reminder（方案A）
2. **治标**：执行层兜底删除非法参数（方案C，已实施）
3. **辅助**：加强制性约束提醒（方案B，可选）
...
| 方式 | 优点 | 缺点 |
|------|------|------|
| Parameter Reminder | 参数名集中、LLM调用时记忆清晰 | 信息简略、易混淆 |
| Tool描述中直接写 | 参数完整、带默认值 | 信息分散、LLM需翻阅大段文字 |
...
LLM调用时会参考这行描述，但**仍有风险**：
- 段落3有4500+字符，LLM可能记不住具体参数名
- 无集中的Parameter Reminder，调用时需翻阅大段文字

**建议**：File类型也应增加Parameter Reminder，格式如：
```
Parameter Reminder:
- read_text_file: file_path(必填), head/tail/offset/limit/encoding(可选)
- write_text_file: file_path(必填), text(必填,实际内容), mode/encoding/backup(可选)
- list_directory: dir_path(必填), include_hidden/max_depth(可选)
```
...
| **总字符数** | 7182 | 8659 | **3831** |
| **功能段落数** | 6个 | 5个 | **6个** |
| **Parameter Reminder** | ✓ 有（简略） | ✗ 无 | **✓ 有（完整）** |
...
### 8.2 Document sys_prompt 结构（3831字符，6个功能段落）
...
| 类型 | 参数数量 | 标注required/optional | 带类型说明 | 带示例 |
|------|---------|---------------------|----------|--------|
| **Time** | 10个工具 | ✗ 只有参数名 | ✗ 无 | ✗ 无 |
| **File** | N/A | **✗ 无Parameter Reminder** | N/A | N/A |
| **Document** | 13个工具 | **✓ 标注required** | ✗ 无 | ✗ 无 |

**Document的Parameter Reminder是三者中最好的**：
- 明确标注`required`
- 参数名准确
- 但仍缺少类型说明和示例
...
|------|------|------|----------|
| **sys_prompt长度** | 7182 | 8659 | **3831** |
| **task_prompt长度** | 274 | 452 | **203** |
| **Parameter Reminder** | 有（简略） | **无** | 有（完整） |
| **特殊规则段落** | 无 | 有(2792字符) | 无 |
| **工具参数复杂度** | 中 | 高 | **低** |
| **参数命名风险** | 中 | 高 | **低** |
...
|--------|---------|----------|------|
| **总字符数** | 7177 | 4611 | Shell少2566字符 |
| **功能段落数** | 6个 | **5个** | Shell少1个 |
| **Parameter Reminder** | ✓ 有（简略） | **✗ 无** | **关键差异！** |
| **Tool详细描述** | ~3500字符 | ~720字符 | Shell更精简 |
| **Tool Call Examples** | ~500字符 | ~1500字符 | Shell示例更多 |
| **Safety提醒** | ~100字符 | ~1380字符 | Shell安全提醒更详细 |

### 9.2 Shell sys_prompt 结构（4611字符，5个功能段落）
...
#### 差异1：Shell无Parameter Reminder

**Shell工具参数简单**：
```
execute_shell_command(command, timeout, working_dir)
...
参数多为`command`/`path`单参数，LLM不易混淆。但仍建议补充：
```
Parameter Reminder:
- execute_shell_command: command(required,命令字符串), timeout(optional), working_dir(optional)
- change_directory: path(required,目录路径)
- check_command_available: command(required,命令名)
```
...
|--------|---------|------------|------|
| **总字符数** | 7177 | 5020 | Network少2157字符 |
| **功能段落数** | 6个 | **6个** | 相同 |
| **Parameter Reminder** | ✓ 有（简略） | **✓ 有（详细）** | Network更详细 |
| **Tool详细描述** | ~3500字符 | ~700字符 | Network更精简 |
| **Tool Call Examples** | ~500字符 | ~1400字符 | Network示例更多 |
| **Safety提醒** | ~100字符 | ~700字符 | Network安全提醒更详细 |

### 10.2 Network sys_prompt 结构（5020字符，6个功能段落）
...
### 10.3 Network task_prompt（88字符，**最简略**）
...
**Network的task_prompt最简略**：只有一句泛化的"think step by step"，无具体操作步骤指导。

### 10.4 关键差异分析

#### 差异1：Network Parameter Reminder最详细

**Network的Parameter Reminder是所有类型中最好的**：
...
| 类型 | 标注required/optional | 带默认值 | 带类型说明 |
|------|---------------------|---------|----------|
| Time | ✗ 只有参数名 | ✗ | ✗ |
| File | **✗ 无Parameter Reminder** | N/A | N/A |
| Document | ✓ 标注required | ✗ | ✗ |
| Shell | **✗ 无Parameter Reminder** | N/A | N/A |
| **Network** | **✓ 标注required/optional** | **✓ 带default值** | ✗ |

**Network最好**：同时标注了required/optional和default值。

#### 差异2：Network task_prompt过于简略（88字符）

**对比**：
```
Time (274字符):   3步详细 + Current time
Shell (212字符):  3步详细（含命令检查）
Network (88字符): 1步泛化 "think step by step"
```

**风险**：Network工具使用场景多样（HTTP/下载/搜索/Ping），泛化指导不足以帮助LLM选择正确工具。

**建议**：Network task_prompt应补充步骤指导：
```
Task: {task}
Please help me complete this network task. Follow these steps:
1. First, determine what type of network operation is needed
2. Use the appropriate network tool (http_request/download_file/search_web/ping/port_check)
3. Handle any errors and provide a clear result
```
...
| 维度 | Time | File | Document | Shell | Network |
|------|------|------|----------|-------|---------|
| **字符数** | 274 | 452 | 203 | 212 | **88** |
| **Current time** | ✓ | ✓ | ✗ | ✗ | ✗ |
| **步骤指导** | 3步详细 | 4步+Remember | 3步 | 3步 | **1步泛化** |
