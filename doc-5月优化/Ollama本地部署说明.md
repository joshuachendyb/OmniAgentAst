...
| v1.3 | 2026-05-23 00:29:31 | 小健 | 修复 §5.2 模型路径仍写 C 盘（改为 E 盘）；修复 §6.1 说"没生效"（改为"已生效"）；更新底部时间 |
...
旧路径 C:\Users\chend\.ollama\models\ 已清空
（Ollama 会重建空目录，但里面没有模型文件）
```
...
**已生效**。`OLLAMA_MODELS` 设为 `E:\ollama\models` 后，模型从 E 盘正常加载，全部 7 个 Ollama 测试通过。
