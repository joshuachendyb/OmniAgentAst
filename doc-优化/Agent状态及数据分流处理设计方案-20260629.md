...
| `react_cycle.py` | 修改 | 9 处 `agent.status = X`/`agent.set_xxx` 全部调 `status_table` 函数。`_dispatch_handler` 改为 seen_types+last_event+recoverable 推断（可恢复→SUSPENDED）。新增 `handle_llm_error`。新增 `_handle_unknown`。`_process_single_step` 不再读 Send() 返回值。重试机制：SUSPENDED→while 循环+max_retries=3 | 9 处替换 + 2 处新增 + 3 处重试逻辑 | §4.7 |
...
**改动统计**：新建 1 文件，修改 8 文件，删除 3 个 base_agent 旧方法，新增 2 个 handler 函数 + 1 个 SUSPENDED 状态，替换 9 处 set_xxx/status= 调用，3 处原 RETRYABLE_ERROR 改为 SUSPENDED 重试逻辑，4 处 message_builder.add_xxx 删除。
