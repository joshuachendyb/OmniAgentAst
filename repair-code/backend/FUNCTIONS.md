...
**最后更新时间**: 2026-08-02 22:20:24
**维护人**: 小沈 (v2.1 语法护栏由 小欧 重建于 2026-07-21)

## 六、语法护栏（app/tools/fundamental/syntax_validator.py）【v2.1新增 — 小欧 2026-07-21】

| 函数名 | 功能 | 参数 | 返回值 |
|--------|------|------|--------|
| `validate_python_syntax` | 校验Python语法；BOM去扰(BOM-002)+BUG-002；无str(e)截断(调用方自行决定) | content: str, file_path: Optional[str]=None | Optional[str] 错误信息或None |
...
| v2.1 | 2026-07-21 | 新增 语法护栏 章节（app/tools/fundamental/syntax_validator.py）: validate_python_syntax + _strip_bom（从edit_text_file抽出,BOM去扰+BUG-002,复用write_text_file+tool_fc_helper） | 小欧 |
