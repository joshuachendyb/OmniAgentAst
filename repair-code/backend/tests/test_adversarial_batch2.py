...
# 小欧 2026-07-13: TestEnsureFailedFinalStep 已删除 — _ensure_failed_final_step 在 v3.2
# 终态统一约定中已移除(失败终态仅 ErrorStep 表示, 不再补发 FinalStep), 旧测试断言已失效。
...
class TestDispatchHandler:
    ...
    async def test_unknown_type_no_content(self):
        ...
        events = []
        async for ev in _dispatch_handler(agent, {"type": "unknown_type", "content": ""}):
            events.append(ev)
        # 未知类型应设FAILED
        assert len(events) >= 1
    ...
    async def test_unknown_type_with_content(self):
        ...
        agent.llm_call_count = 1
        events = []
        async for ev in _dispatch_handler(agent, {"type": "unknown_type", "content": "invalid response"}):
            events.append(ev)
        assert len(events) >= 1
...
class TestErrorHandler:
    def test_unknown_error(self):
        from app.services.agent.react_cycle import handle_react_error, _is_recoverable_error
        agent = _make_error_agent()
        step = handle_react_error(agent, ValueError("bad value"), 5)
        assert step.error_type == "unknown"
        # 小欧 2026-07-13: ErrorStep 已无 recoverable 字段; 可恢复性由 _is_recoverable_error 在重试层判定
        assert _is_recoverable_error(ValueError("bad value")) is False

    def test_fc_format_error(self):
        from app.services.agent.react_cycle import handle_react_error, _is_recoverable_error
        from app.services.llm.core import LLMResponseError
        agent = _make_error_agent()
        step = handle_react_error(agent, LLMResponseError(message="format fail"), 3)
        assert step.error_type == "server"
        # 小欧 2026-07-13: LLMResponseError 仍属可恢复错误(系统重试层), 但不再存于 ErrorStep.recoverable
        assert _is_recoverable_error(LLMResponseError(message="format fail")) is True
...
class TestFinalizeCycle:
    def test_finalize_cycle_ok(self):
        from app.services.agent.react_cycle import _finalize_cycle
...
class TestChunkBufferAdversarial:
    ...
    def test_should_force_stop_exact(self):
        from app.services.agent.chunk_buffer import ChunkBuffer
        cb = ChunkBuffer(max_consecutive=5, max_chunks_before_stop=10)
        for i in range(10):
            cb.append(str(i))
...
class TestLLMResponseError:
    def test_fc_format_error_sets_failed(self):
        ...
        from app.services.llm.core import LLMResponseError
        agent = _make_error_agent()
        step = handle_react_error(agent, LLMResponseError(message="bad"), 2)
        assert step.error_type == "server"
        # 小欧 2026-07-13: 可恢复性改由 _is_recoverable_error 判定, ErrorStep 不再含 recoverable
        from app.services.agent.react_cycle import _is_recoverable_error
        assert _is_recoverable_error(LLMResponseError(message="bad")) is True
...
class TestBuildCallListEdge:
    def test_tool_name_empty_but_pending_has_name(self):
        ...
        assert len(r.all_calls) == 2  # 主调用(空名) + pending(有名字)
    ...
    def test_pending_call_with_null_tool_params(self):
        ...
        assert r.all_calls[1]["tool_params"] == {}
...
def _make_finalize_agent():
    ...
    return type("MockAgent", (), {
        ...
        "message_builder": type("MockMB", (), {"pop_temp_messages": lambda self: 0})(),
        "_step_emitter": type("MockEmitter", (), {
            "complete_task": lambda self, s: None,
        })(),
    })()
