"""全链路E2E集成测试 - unit-06: 任务书执行验证(task003)
...
@pytest.mark.asyncio
async def test_e2e_unit_06_task_execution():
    """unit-06: 任务书执行 - 读取任务书(task003.txt)逐一执行全部任务"""
    ...
    try:
        ...
        assert len(db_read_steps) > 0, "DB steps中应有readtext操作(MUST unit-06)"
