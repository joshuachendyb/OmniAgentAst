"""全链路E2E集成测试 - unit-05: 任务书执行验证(task002)
...
@pytest.mark.asyncio
async def test_e2e_unit_05_task_execution():
    """unit-05: 任务书执行 - 读取任务书(task002.txt)逐一执行全部任务"""
    ...
    try:
        ...
        # unit-05核心: 必须调用readtext读取任务书
        tool_names = [t["tool_name"] for t in result["tool_calls"]]
        file_tools = {"readtext"}
        has_read = any(n in file_tools for n in tool_names)
        assert has_read, f"必须调用readtext(MUST unit-05), 实际: {tool_names}"

        # unit-05核心: 回复应包含任务总结关键词
        ...
        assert has_task_keyword, f"回复应包含任务总结关键词(MUST unit-05), 回复: {resp[:200]}"
