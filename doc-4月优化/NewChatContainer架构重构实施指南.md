...
### Phase 3: 迁移回调到useChatCallbacks（2天）
...
const NewChatContainer: React.FC = () => {
  ...
  const {
    ...
    chatCallbacks.onStep,      // 替换原有的onStep
    chatCallbacks.onChunk,     // 替换原有的onChunk
    chatCallbacks.onComplete,  // 替换原有的onComplete
    chatCallbacks.onError,     // 替换原有的onError
    chatCallbacks.onPaused,    // 替换原有的onPaused
    chatCallbacks.onResumed    // 替换原有的onResumed
  );
};
```
...
### Phase 5: 完善useChatSession Hook（1天）

#### Task 5.1: 完善会话管理Hook
```typescript
// 更新：src/hooks/chat/useChatSession.ts
// 包含会话加载、创建、清空、标题更新等功能
```
