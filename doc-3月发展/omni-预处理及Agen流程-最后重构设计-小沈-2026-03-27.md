...
| v2.89 | 2026-03-27 15:57:01 | 附录6更新为已实施版本：修正架构为chat保留在router，react_sse_wrapper只处理动作意图，更新实施步骤为实际代码，添加架构决策记录 |
...
实现分层架构：
- 简单对话：chat_router → chat_stream_query
- 动作意图：chat_router → react_sse_wrapper → Agent

**当前状态**（2026-03-27 15:57 完成）：
- chat_router.py ✅ 已实现6步流程 + 意图分发
- file_react.py ✅ 已删除ver1_run_stream，保留run_stream
- react_sse_wrapper.py ✅ 已实现参数接收和动作意图分发
- chat_stream_query ✅ 保留在router里
- 参数传递 ✅ router传递running_tasks/running_tasks_lock/current_execution_steps
...
intent_type + confidence
    ↓
    ├─ intent_type == "chat" 且 confidence >= 0.3
    │   └── router → _handle_chat_operation → chat_stream_query
    │
    └─ 其他意图 (file/network/desktop 或 confidence < 0.3)
        └── router → generate_sse_stream → react_sse_wrapper
            ├─ intent_type == "file" → FileReactAgent.run_stream()
            ├─ intent_type == "network" → NetworkReactAgent (待实现)
            └─ intent_type == "desktop" → DesktopReactAgent (待实现)
```

**架构说明**：
- chat_stream_query 保留在 router 里（参数都在router准备）
- react_sse_wrapper 只处理动作意图（file/network/desktop）
- 避免重复：router传递参数，react_sse_wrapper不再重复创建

---

#### 3.2 实施步骤（已实现，2026-03-27）

**第一步：修改 file_react.py** ✅ (2026-03-27)
- 删除 `ver1_run_stream()` 方法
- 保留 `run_stream()` 方法（返回 event dict）
- SSE 格式化逻辑移到 react_sse_wrapper

**第二步：修改 react_sse_wrapper.py** ✅ (2026-03-27)
...
async def generate_sse_stream(
    ...
    running_tasks: Optional[Dict[str, Any]] = None,        # 新增
    running_tasks_lock: Optional[asyncio.Lock] = None,      # 新增
    current_execution_steps: Optional[List[Dict]] = None   # 新增
):
```
...
**删除重复创建代码**：
- 删除 `next_step = create_step_counter()`（覆盖传入的值）
- 删除 `current_execution_steps: List[Dict] = []`（覆盖传入的值）
...
if intent_type == "file" and confidence >= 0.3:
    ...
    # 兜底（理论上不会走到这里，chat已由router处理）
    yield create_error_response(...)
```

**第三步：修改 chat_router.py** ✅ (2026-03-27)
...
**删除无用代码**：
- 删除第253-257行的 llm_client（无用的，react_sse_wrapper自己创建）

**第四步：验证** ✅ (2026-03-27)
- Python语法检查：通过
- test_agent.py：20 passed
- test_adapter.py：32 passed

---

#### 3.3 架构决策记录

**决策**：chat_stream_query 保留在 router 里

**原因**：
1. chat_stream_query 需要15个参数，参数准备都在 router 里
2. 传入 react_sse_wrapper 需要传递所有参数，增加复杂度
3. 最小改动原则：只让动作意图走 react_sse_wrapper

**参数传递设计**：
| 参数 | chat_router创建 | react_sse_wrapper接收 |
|------|----------------|---------------------|
| running_tasks | ✅ | ✅ 使用传入值 |
| running_tasks_lock | ✅ | ✅ 使用传入值 |
| current_execution_steps | ✅ | ✅ 使用传入值 |
| next_step | ✅ | ✅ 使用传入值（不再重复创建）|
...
| 原因 | 说明 |
|------|------|
| **职责分离** | 简单对话和动作意图处理逻辑不同 |
| **参数复用** | router准备的参数直接传给处理函数 |
| **最小改动** | 只改必要的部分，不动已稳定的代码 |
| **可扩展性** | 未来添加network/desktop只需修改react_sse_wrapper |

---

## 附录6.2：阶段6实施记录

**实施时间**：2026-03-27 15:57:01

| 步骤 | 文件 | 改动 | 状态 |
|------|------|------|------|
| 第一步 | file_react.py | 删除ver1_run_stream，保留run_stream | ✅ |
| 第二步 | react_sse_wrapper.py | 添加参数，删除重复创建，添加分发逻辑 | ✅ |
| 第三步 | chat_router.py | 意图分发，无用代码删除 | ✅ |
| 第四步 | 验证 | 语法+单元测试 | ✅ |

**Commit**：3e18f8c2 feat: chat_router意图分发+react_sse_wrapper参数优化-小沈-2026-03-27
